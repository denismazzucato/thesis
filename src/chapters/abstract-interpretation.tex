...
%   The \emph{forward reachability state semantics} $\forwardreachabilitysemanticsnoparam\in\forwardreachabilitytype$ collects the states that are reachable from a given set of initial states $\initialstates\in\setof\state$ by abstraction of the maximal trace semantics $\tracesemanticsnoparam\in\tracetype$. The two form a Galois connection:
%   \begin{align*}
%     \tuple{\tracetype}{\tracesubseteq}\galois{\forwardreachabilityabstraction}{\forwardreachabilityconcretization}\tuple{\forwardreachabilitytype}{\forwardreachabilitysubseteq}
%   \end{align*}
%   where the abstraction $\forwardreachabilityabstraction$ collects all the states that are reachable from the initial states:
%   \begin{align*}
%     \forwardreachabilityabstraction(\defsetoftraces) &\DefeQ \initialstates \setjoin \setdef{\defstate\in\state}{\exists \defstate'\in\initialstates,\deftrace\in\finitesequences, \deftrace'\in\finiteinfinitesequences.\spacer \concat{\concat{\defstate'}{\deftrace}}{\concat{\defstate}{\deftrace'}} \in \defsetoftraces}
%   \end{align*}
% Thus, we obtain the following definition for the forward reachability state semantics:
% \[ \forwardreachabilitysemanticsnoparam \DefeQ \forwardreachabilityabstraction(\tracesemanticsnoparam) \]
% It can be specified in fixpoint form as follows:
% \begin{align*}
%   \forwardreachabilitysemanticsnoparam &\spacearound{=} \lfp^{\forwardreachabilitysubseteq} \forwardreachabilitytransformer \\
%   \forwardreachabilitytransformer(\defsetofstates) &\spacearound{=} \initialstates \setjoin \setdef{\defstate\in\state}{\exists \defstate'\in\defsetofstates.\spacer \tuple{\defstate'}{\defstate} \in \transitionrelation}
% \end{align*}
% Note that, while the traces from the maximal trace semantics are built backwards, \cf{} \refthm*{maximal-trace-semantics}, the states belonging to the forward reachability state semantics are retrieved forwards from the initial states $\initialstates$.
% \end{example}
...
\begin{marginfigure}
  \begin{align*}
    &\aexpsemantics \IN \setof\env \to \setof\values \\
    &\aexpsemanticsnoparam \semanticsof{\defvalue} \defsetofenvs \DefeQ \{\defvalue\} \\
    &\aexpsemanticsnoparam \semanticsof{\defvariable} \defsetofenvs \DefeQ \setdef{\defenv(\defvariable)}{\defenv\in\defsetofenvs} \\
    &\aexpsemanticsnoparam \semanticsof{\defaexp + \defaexp'} \defsetofenvs \DefeQ \\
      & \quad \setdef*{\defvalue + \defvalue'}{\defvalue \in \aexpsemanticsnoparam \semanticsof{\defaexp}\defsetofenvs \land {} \\ \defvalue' \in \aexpsemanticsnoparam \semanticsof{\defaexp'}\defsetofenvs} \\
    &\aexpsemanticsnoparam \semanticsof{\defaexp - \defaexp'} \defsetofenvs \DefeQ \\
      & \quad \setdef*{\defvalue - \defvalue'}{\defvalue \in \aexpsemanticsnoparam \semanticsof{\defaexp}\defsetofenvs \land {} \\ \defvalue' \in \aexpsemanticsnoparam \semanticsof{\defaexp'}\defsetofenvs} \\
    &\aexpsemanticsnoparam \semanticsof{\randval(l, u)} \defsetofenvs \DefeQ \\
...
\begin{marginfigure}
  \begin{align*}
    &\bexpsemantics \IN \setof\env \to \setof\env \\
    &\bexpsemanticsnoparam \semanticsof{\defaexp \le \defvalue} \defsetofenvs \DefeQ \\
      & \quad \setdef{\defenv\in\defsetofenvs}{\defenv(\defaexp) \le \defvalue} \\
    &\bexpsemanticsnoparam \semanticsof{\defaexp = \defvalue} \defsetofenvs \DefeQ \\
      & \quad \setdef{\defenv\in\defsetofenvs}{\defenv(\defaexp) = \defvalue} \\
    &\bexpsemanticsnoparam \semanticsof{\defbexp \land \defbexp'} \defsetofenvs \DefeQ \\
      & \quad \bexpsemanticsnoparam \semanticsof{\defbexp}\defsetofenvs \setmeet \bexpsemanticsnoparam \semanticsof{\defbexp'}\defsetofenvs \\
    &\bexpsemanticsnoparam \semanticsof{\lnot \defbexp} \defsetofenvs \DefeQ \defsetofenvs \setminus \bexpsemanticsnoparam \semanticsof{\defbexp}\defsetofenvs
  \end{align*}
...
\begin{subfigure}{.45\textwidth}
  \begin{align*}
    ...
    &\quad\finalloc[\plwhilestmt] \DefeQ \\
...
The transition relation $\transitionrelation\in\pair\state\state$ applied to a program $\defprogram$ is defined as $\transitionrelation\semanticsof{\defprogram}\in\pair\state\state$ in \reffig{transition-semantics}. The semantics of a skip statement $\skipstmt$ is a simple step forward from one program location to the next one, without modifying the set of environments.
...
\begin{figure}[t]
  \begin{align*}
    ...
    &\transitionrelation \semanticsof{\plassertstmt} \DefeQ \setdef{\tracestep{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\finalloc[\plassertstmt]}{\defenv}}}{\defenv\in\env \land \defenv \in \bexpsemanticsnoparam \semanticsof{\defbexp}\{\defenv\}} \\
    &\transitionrelation \semanticsof{\plifstmt} \DefeQ \\
      &\quad \setdef{\tracestep{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\initialloc[\defstmt]}{\defenv}}}{\defenv\in\env \land \defenv \in \bexpsemantics \{\defenv\}} \SetjoiN \transitionrelation \semanticsof{\defstmt} \SetjoiN {} \\
      &\quad \setdef{\tracestep{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\initialloc[\defstmt']}{\defenv}}}{\defenv\in\env \land \defenv \notin \bexpsemantics \{\defenv\}} \SetjoiN \transitionrelation \semanticsof{\defstmt'} \\
    &\transitionrelation \semanticsof{\plwhilestmt} \DefeQ \\
      &\quad \setdef{\tracestep{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\initialloc[\defstmt]}{\defenv}}}{\defenv\in\env \land \defenv \in \bexpsemantics \{\defenv\}} \SetjoiN \transitionrelation \semanticsof{\defstmt} \SetjoiN {} \\
      &\quad \setdef{\tracestep{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\finalloc[\plwhilestmt]}{\defenv}}}{\defenv\in\env \land \defenv \notin \bexpsemantics \{\defenv\}} \\
...
\begin{figure*}[t]
  \begin{align*}
    ...
    &\tracesemanticsnoparam \semanticsof{\plassertstmt}\defsetoftraces \DefeQ
      \setdef*{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\finalloc[\plassertstmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \defenv \in \bexpsemantics\{\defenv\} \land \deftrace\in\finiteinfinitesequences \\ {} \land \concat{\tracetuple{\finalloc[\plassertstmt]}{\defenv}}{\deftrace}\in\defsetoftraces} \\
    &\tracesemanticsnoparam \semanticsof{\plifstmt}\defsetoftraces \DefeQ \\
      &\quad \setdef*{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \defenv \in \bexpsemantics\{\defenv\} \land \deftrace\in\finiteinfinitesequences \\ {} \land \concat{\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}\in\tracesemanticsnoparam\semanticsof{\defstmt}\defsetoftraces} \SetjoiN {} \\
      &\quad \setdef*{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\initialloc[\defstmt']}{\defenv}}{\deftrace}}{\defenv\in\env \land \defenv \notin \bexpsemantics\{\defenv\} \land \deftrace\in\finiteinfinitesequences \\ {} \land \concat{\tracetuple{\initialloc[\defstmt']}{\defenv}}{\deftrace}\in\tracesemanticsnoparam\semanticsof{\defstmt'}\defsetoftraces} \\
    &\tracesemanticsnoparam \semanticsof{\plwhilestmt}\defsetoftraces \DefeQ \lfp^{\tracesubseteq} \tracetransformer \\
      &\quad \tracetransformer(X) \DefeQ \setdef*{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \defenv \in \bexpsemantics\{\defenv\} \land \deftrace\in\finiteinfinitesequences \\ {} \land \concat{\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}\in\tracesemanticsnoparam\semanticsof{\defstmt}X} \SetjoiN {} \\
        &\qquad \setdef*{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\finalloc[\plwhilestmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \defenv \notin \bexpsemantics\{\defenv\} \\ {} \land \concat{\tracetuple{\finalloc[\plwhilestmt]}{\defenv}}{\deftrace}\in\defsetoftraces} \\
...
We introduce the \emph{forward reachability state semantics} $\forwardreachabilitysemantics\in\forwardreachabilitytype$, given a set of states that hold before the execution (\ie, a precondition), relates the set of states that are reached after executing the program $\defprogram$ (\ie, a postcondition). \reffig{forward-reachability-state-semantics} present the semantics by structural induction on the program.

Missing:
\begin{itemize}
  \item Definition of forward and backward reachability concrete semantics in the previous section.
  \item Definition here of the forward and backward reachability semantics by structural induction.
  \item Next chapter shows abstract domain and forward \& backward reachability abstract semantics.
\end{itemize}
