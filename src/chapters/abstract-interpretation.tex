...
which provide a sound and computable over-approximation of the concrete semantics.
  We introduce the concept of abstract domains with a simple example of the integer interval abstract domain \sidecite{Cousot1977}, called \intervalname. This domain abstracts the values of each variable with an interval. We assume concrete values are integers. The interval values are defined as:
  \begin{align*}
  ...
  To obtain the interval abstract domain, we lift the intervals defined above as functions from variables $\defvariable$ to intervals $\intervaltype$:
  \begin{align*}
    \intervalabstractdomaintype \DefeQ \variables \to (\intervaltype \setminus \{\intervalbottom\}) \setjoin \{\intervalbottom\}
  \end{align*}
  where we enforce a single representation for the bottom element $\intervalbottom$, that is, if the interval associated to a variable is empty, then abstract domain contains no value for all the variables. The partial order $\intervalabstractdomainsubseteq$ is defined as:
  \begin{align*}
    \forall \defabstractvalue_1, \defabstractvalue_2.\spacer \defabstractvalue_1 = \intervalabstractdomainbottom \lor (\defabstractvalue_1 \neq \intervalabstractdomainbottom \land \defabstractvalue_2 \neq \intervalabstractdomainbottom \land \forall \defvariable \in \variables.\spacer \defabstractvalue_1(\defvariable) \intervalsubseteq \defabstractvalue_2(\defvariable))
  \end{align*}
  THen, the concretization function $\intervalabstractdomainconcretization\in\intervalabstractdomaintype\to\setof\env$ is defined as:
  \begin{align*}
  ...
  \end{align*}
  Fortunately, the integer\sidenote{In case of \dots, it would not be the case.} interval domain enjoys a Galois connection with the following abstraction function $\intervalabstractdomainabstraction\in\setof\env\to\intervalabstractdomaintype$:
  \begin{align*}
    \intervalabstractdomainabstraction(\defenv) \DefeQ \lambda \defvariable\in\variables.\spacer \begin{cases}
      \intervalbottom & \text{if } \defenv(\defvariable) \notin \Z \\
      \interval{\defenv(\defvariable)}{\defenv(\defvariable)} & \text{otherwise}
    \end{cases}
  \end{align*}
  It holds that:
  \begin{align*}
