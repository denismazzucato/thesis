...
\begin{marginfigure}
  \begin{tikzpicture}
    % new length: first line should be around 1.2
    \newlength{\first}
    \setlength{\first}{1.2cm}

    % Nodes representing elements in the interval domain
    \node (top) at (3, 4) {$[-\infty,+\infty]$};
    \node (A) at (1, \first) {$[-2,-2]$};
...
\begin{definition}[Numerical Abstract Domain]\labdef{abstract-domain}
  ...
  \begin{itemize}
    ...
    \item a sound widening $\abstractdomainwidening\in\pair\abstractdomain\abstractdomain\to\abstractdomain$ operator, \cf{} \refdef{widening};
...
Different abstract domains represent complementary information about the program variables. Therefore, it is common to combine multiple abstract domains to leverage the strengths of each domain. The combination of abstract domains is known as the \emph{reduced product} domain \sidecite{Cousot1979a}. The reduced product domain is defined as the Cartesian product of the abstract domains, with the ordering defined as the pointwise ordering of the abstract elements. The reduced product domain is a sound abstraction of the concrete domain, and it is more precise than any of the individual domains. However, the reduced product domain is also more expensive to compute than the individual domains. The reduced product domain is particularly useful when the individual domains are complementary, \eg{} when one domain is good at inferring linear relationships and another domain is good at inferring disjunctions.
...
\begin{description}
  ...
  \marginnote{
    \begin{align*}
      ...
      &\quad \abstractdomainfilter \semanticsof{\defbexp}\defabstractvalue \abstractdomainjoin \abstractdomainfilter \semanticsof{\neg\defbexp}\defabstractvalue
    \end{align*}
  }
  ...
  \marginnote{
    \begin{align*}
      ...
      &\quad \lfp^{\abstractdomainsubseteq} \abstractdomainwidening \circ \abstractdomainfilter \semanticsof{\defbexp} \circ \abstractdomainassign \semanticsof{\defstmt}
    \end{align*}
  }
  \item[\normalfont ($\pwhilestmt$)] The abstract semantics of a while statement is the least fixpoint of the widening of the abstract semantics of the loop body, filtered by the loop condition.
  \marginnote{
    \begin{align*}
      &\forwardsemanticsnoparam \semanticsof{\pcompstmt}\defabstractvalue \DefeQ \forwardsemanticsnoparam \semanticsof{\defstmt'}(\forwardsemanticsnoparam \semanticsof{\defstmt}\defabstractvalue)
    \end{align*}
  }
  \item[\normalfont ($\pcompstmt$)] The abstract semantics of a composition statement is the composition of the abstract semantics of the two statements.
  \item[\normalfont ($\pprogstmt$)] The abstract semantics of a program is the abstract semantics of the statement with the bottom element as initial abstract element.
    \begin{definition}[Forward Reachability State Abstract Semantics]
      ...
      \begin{align*}
        \forwardsemantics(\defprogram) \DefeQ \forwardsemanticsnoparam \semanticsof{\defstmt}\intervalabstractdomainbottom
      \end{align*}
    \end{definition}
\end{description}
...
The backward reachability state abstract semantics $\backwardsemantics\in\backwardtype$ is defined by induction on the structure of the program, as follows:
...
\begin{description}
  ...
  \marginnote{
    \begin{align*}
      &\backwardsemanticsnoparam \semanticsof{\passignstmt}\defabstractvalue \DefeQ \abstractdomainassign \semanticsof{\assignstmt}\defabstractvalue
    \end{align*}
  }
  \item[\normalfont ($\plassignstmt$)] The abstract semantics of an assignment statement is the abstract assignment of the abstract element.
  \marginnote{
    \begin{align*}
      ...
      &\quad \abstractdomainfilter \semanticsof{\defbexp}\defabstractvalue \abstractdomainmeet \abstractdomainfilter \semanticsof{\neg\defbexp}\defabstractvalue
    \end{align*}
  }
  \item[\normalfont ($\pifstmt$)] The abstract semantics of a conditional statement is the meet of the abstract semantics applied to the two branches.
  \marginnote{
    \begin{align*}
      &\backwardsemanticsnoparam \semanticsof{\pwhilestmt}\defabstractvalue \DefeQ \\
      &\quad \gfp^{\abstractdomainsubseteq} \abstractdomainmeet \circ \abstractdomainfilter \semanticsof{\defbexp} \circ \abstractdomainassign \semanticsof{\defstmt}
    \end{align*}
  }
  \item[\normalfont ($\pwhilestmt$)] The abstract semantics of a while statement is the greatest fixpoint of the meet of the abstract semantics of the loop body, filtered by the loop condition.
  \marginnote{
    \begin{align*}
      &\backwardsemanticsnoparam \semanticsof{\pcompstmt}\defabstractvalue \DefeQ \backwardsemanticsnoparam \semanticsof{\defstmt}\left(\backwardsemanticsnoparam \semanticsof{\defstmt'}
  ...
  \item[\normalfont ($\pprogstmt$)] The abstract semantics of a program is the abstract semantics of the statement with the bottom element as initial abstract element.
    \begin{definition}[Backward Co-Reachability State Abstract Semantics]
      ...
      \begin{align*}
        \backwardsemantics(\defprogram) \DefeQ \backwardsemanticsnoparam \semanticsof{\defstmt}\intervalabstractdomainbottom
      \end{align*}
    \end{definition}
\end{description}
