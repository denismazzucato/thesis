...
\begin{marginfigure}
  \begin{align*}
    ...
    \defbexp \grammardef \defaexp \leq \defvalue \\
...
%
Boolean expressions $\defbexp$ are either a comparison between arithmetic values $\defaexp \leq \defvalue$ and $\defaexp = \defvalue$, a conjunction $\defbexp \land \defbexp$, or a negation $\lnot \defbexp$.
...
The set $\state$ is a (potentially infinite) set of program states, mapping tuples of variables and locations to values.


\begin{definition}[State]\labdef{state}
  The set of \emph{program states} $\state$ is defined as:
  \begin{align*}
    \state \IN \pair\variables\locs \to \values
  \end{align*}
\end{definition}
