...
This domain abstracts the values of each variable with an interval. The interval values are defined as:
  ...
  The abstract semantics of an assignment statement $\plassignstmt$ is defined as:
  \begin{align*}
    \intervalabstractdomainassign(\defabstractvalue, \defvariable, \defaexp) \DefeQ \begin{cases}
...
Next, we show how the (sound) operators of the numerical abstract domains can be used to define the abstract semantics of programs. We present both the forward and backward reachability state abstract semantics for a generic numerical abstract domain, \cf{} \refdef{abstract-domain}.
