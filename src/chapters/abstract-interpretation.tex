...
\emph{Dans ce chapitre, nous définissons les notations et introduisons les concepts nécessaires pour la théorie des ensembles et la logique afin comprendre le reste de la thèse. Cela présuppose une connaissance de base des mathématiques. Par la suite, nous présenterons un aperçu de l'interprétation abstraite.}
...
We can state properties by logical predicates\sidenote{For instance, $x \le x + 1$ is a basic logical predicate} yielding a Boolean value.
...
\begin{example}
  Let us consider the partially ordered set $\langle \setof{\{a, b, c\}}, \subseteq \rangle$ and the subset $X' \defeq \{\{a\}, \{b\}, \{c\}, \{a, b\}, \{a, c\}, \{b, c\}\}$, represented in \reffig{elements}.
  The maximal elements of $X'$ are $\{a, b\}$, $\{a, c\}$, and $\{b, c\}$, and are not comparable with each other.
  Hence, the maximum of $X'$ does not exist, however, its least upper bound is $\join X' = \{a, b, c\}$.
  The minimal elements of $X'$ are $\{a\}$, $\{b\}$, and $\{c\}$.
  Again the minimum of $X'$ does not exist and its greatest lower bound is $\meet X' = \emptyset$.
\end{example}
...
We refer to a function as \emph{increasing} monotone when $x \sqsubseteq_X x' \implies f(x) \sqsubseteq_Y f(x')$ and \emph{decreasing} monotone when the inequality is reversed, \ie, $x \sqsubseteq_X x' \implies f(x') \sqsupseteq_Y f(x)$. Note that, a decreasing monotone function is increasing monotone over $\langle X, \sqsubseteq_Y \rangle$ and $\langle Y, \sqsupseteq_Y \rangle$, where $\sqsupseteq_Y$ is the reverse order of $\sqsubseteq_Y$.

\begin{definition}[Scott Continuity]\labdef{continuity}
...
\emph{Abstract Interpretation} \sidecite{Cousot1977,Cousot1978} is a general theory for approximating the semantics of programs as a unifying framework for static program analysis, co-invented in the late 1970s by Patrick Cousot and Radhia Cousot.
...
The maximal trace semantics lattice $\tracecompletelattice$ forms a complete lattice.
...
The maximal trace semantics fully describes the behavior of programs. However, reasoning about a particular property of a program is often facilitated by the design of a semantics that abstracts away from irrelevant details about program computations.
...
\index{Collecting Semantics}

\marginnote{Program properties are also referred to as \emph{hyperproperties} \cite{Clarkson2010}. In this thesis, we prefer the term \emph{program properties} to emphasize that we consider properties of the semantics of programs.}
...
Note that, not all program properties can be expressed as trace properties. As an example, the unused property \sidecite{Urban2018} (\cf{} \refch{input-data-usage}) is not a trace property, because whether a program utilizes a variable or not depends on the whole set of traces generated by the program.


\paragraph{Hierarchy of Semantics.}
\index{Hierarchy of Semantics}
...
\begin{example}[Backward Co-Reachability State Semantics]\labexample{backward-reachability-state-semantics}
...
The set $\variables$ is the finite set of (statically allocated) program variables, the data type is $\values \in \{\N, \Z, \R\}$ for arithmetic and $\B$\marginnote{the set of boolean values is $\B \defeq \{\true,\false\}$ where $\true$ is true and $\false$ is false.} for boolean conditions.
The set $\inputvariables \subseteq \variables$ contains the input variables.
\reffig{syntax} shows the syntax of our small imperative language, where $\defvariable\in\variables$ is a variable, $\defvalue\in\values$ is a constant value, $\defaexp$ is an arithmetic expression, $\defbexp$ is a boolean condition, and $\defstmt$ is a program statement.
...
We could have been ever more minimalistic, \eg, all arithmetic expressions could have been defined from $1$, $-$, and iteration, or all boolean conditions with $<$ and $\texttt{nand}$. The syntax in \reffig{syntax} is a trade-off between minimalism, simplicity, and expressiveness.
...
Similarly, we define the semantics of boolean conditions $\defbexp$ as the function $\bexpsemantics\in\env\to\B$ that holds whenever the condition $\defbexp$ holds in the environment $\defenv$. The semantics of boolean conditions is defined in \reffig{bexp-semantics}. In the following, we write $\texttt{true}$ and $\texttt{false}$ for the boolean condition that always evaluates to true and false, respectively.
...
Program states are pairs of a program location and an environment, $\defstate\in\state\defeq\pair\locs\env$.
...
Similarly, the final control point of a program (or a statement) defines where the execution of the program (statement) ends, namely $\finalloc$ and $\finalloc[\defstmt]$ respectively. The formal definitions of initial and final control points are given in \reffig{initial} on the side and \reffig{final} above.
...
% In the following, we lift to set of states and environment the arithmetic expression, boolean condition.
...
\begin{description}
  ...
  \begin{definition}[{Transition Semantics}]\labdef{transition-semantics}
...
\begin{description}
  \marginnote{
    \begin{align*}
      ...
      &\quad\setdef@{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\finalloc[\plskipstmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \deftrace\in\finiteoremptyinfinitesequences\land {} \\ \concat{\tracetuple{\finalloc[\plskipstmt]}{\defenv}}{\deftrace}\in\defsetoftraces}
    \end{align*}}
  ...
  \marginnote{
    \begin{align*}
      ...
      &\quad
       \setdef@{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\finalloc[\plassignstmt]}{\updatemem{\defenv}{\defvariable}{\defvalue}}}{\deftrace}}{\defenv\in\env \land \defvalue \in \aexpsemantics\defenv \land {} \\  \deftrace\in\finiteoremptyinfinitesequences \land {} \\ \concat{\tracetuple{\finalloc[\plassignstmt]}{\updatemem{\defenv}{\defvariable}{\defvalue}}}{\deftrace}\in\defsetoftraces}
    \end{align*}
  }
  ...
  \marginnote{
    \begin{align*}
      ...
      &\quad \setdef@{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \bexpsemantics\defenv \land \deftrace\in\finiteoremptyinfinitesequences \\ {} \land \concat{\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}\in\tracesemanticsnoparam\semanticsof{\defstmt}\defsetoftraces} \setjoin {} \\
      &\quad \setdef@{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\initialloc[\defstmt']}{\defenv}}{\deftrace}}{\defenv\in\env \land \bexpsemanticsnoparam\semanticsof{\neg\defbexp}\defenv \land \deftrace\in\finiteoremptyinfinitesequences \\ {} \land \concat{\tracetuple{\initialloc[\defstmt']}{\defenv}}{\deftrace}\in\tracesemanticsnoparam\semanticsof{\defstmt'}\defsetoftraces}
    \end{align*}
  }
  \item[\normalfont ($\plifstmt$)]
  ...
  \marginnote{
    \begin{align*}
      &\tracesemanticsnoparam \semanticsof{\plwhilestmt}\defsetoftraces \DefeQ \lfp^{\sqsubseteq} \tracetransformer \\
      &\quad \tracetransformer(X) \DefeQ \setdef@{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \bexpsemantics\defenv \land {} \\ \deftrace\in\finiteoremptyinfinitesequences \land {} \\ \concat{\tracetuple{\initialloc[\defstmt]}{\defenv}}{\deftrace}\in\tracesemanticsnoparam\semanticsof{\defstmt}X} \SetjoiN {} \\
        & \setdef@{\concat{\tracetuple{\bigloc}{\defenv}\tracetuple{\finalloc[\plwhilestmt]}{\defenv}}{\deftrace}}{\defenv\in\env \land \bexpsemanticsnoparam\semanticsof{\neg\defbexp}\defenv \land {} \\ \deftrace\in\finiteoremptyinfinitesequences \land {} \\ \concat{\tracetuple{\finalloc[\plwhilestmt]}{\defenv}}{\deftrace}\in\defsetoftraces}
    \end{align*}
  }
  \item[\normalfont ($\plwhilestmt$)]
...
\begin{description}
  ...
  \marginnote{
    \begin{align*}
      ...
      &\quad \forwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt} \setdef*{\tracetuple{\initialloc[\defstmt]}{\defenv}}{\tracetuple{\bigloc}{\defenv}\in\defsetofstates \land {} \\ \bexpsemantics\defenv} \setjoin {} \\
      &\quad \forwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt'} \setdef*{\tracetuple{\initialloc[\defstmt']}{\defenv}}{\tracetuple{\bigloc}{\defenv}\in\defsetofstates \land {} \\ \bexpsemanticsnoparam\semanticsof{\neg\defbexp}\defenv}
    \end{align*}
  }
  \item[\normalfont ($\plifstmt$)] The reachability semantics of a conditional statement is the union of the reachability semantics of the then and the else branch. For the then branch, $\bexpsemantics$ keeps the environments that satisfy the boolean condition $\defbexp$ and update them to the initial location $\initialloc[\defstmt]$, then the reachability semantics of the then branch $\forwardreachabilitytracesemanticsnoparam\semanticsof{\defstmt}$ is computed. Note that, the reachability semantics of the then branch expects control locations at the final location of the then branch (\ie, $\finalloc[\defstmt]$) which is also the final location of the conditional statement, \cf{} \reffig{final}, hence the program locations are not updated after the branch evaluation. Similarly, for the else branch, the reachability semantics of the else branch $\forwardreachabilitytracesemanticsnoparam\semanticsof{\defstmt'}$ is computed for the environments that do not satisfy the boolean condition $\defbexp$.
  \marginnote{
    \begin{align*}
      &\forwardreachabilitytracesemanticsnoparam \semanticsof{\plwhilestmt}\defsetofstates \DefeQ
    \\
    &\quad\setdef@{\tracetuple{\finalloc[\plwhilestmt]}{\defenv}}{\tracetuple{\bigloc}{\defenv} \in \lfp^{\forwardreachabilitytracesubseteq} \forwardreachabilitytracetransformer \land \bexpsemanticsnoparam \semanticsof{\neg\defbexp}\defenv}
    \\
    &\forwardreachabilitytracetransformer(X) \DefeQ \defsetofstates \setjoin {} \\
    &\quad\forwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt} \setdef*{\tracetuple{\initialloc[\defstmt]}{\defenv}}{\tracetuple{\bigloc}{\defenv} \in X \land {} \\ \bexpsemantics\defenv}
    \end{align*}
  }
...
\begin{marginfigure}
  ...
  \begin{align*}
    ...
    \\
    &\forwardreachabilitytracesemanticsnoparam \semanticsof{\pcompstmt}\defsetofenvs \DefeQ \\
    &\quad\forwardreachabilitytracesemanticsnoparam\semanticsof{\defstmt'}(\forwardreachabilitytracesemanticsnoparam\semanticsof{\defstmt}\defsetofenvs)
    \\
...
As a further abstraction, \reffig{forward-reachability-state-semantics-without-location} shows the forward reachability state semantics without explicitly handling program locations, in such case, the program states are just the environments, \ie, $\state = \env$. This semantics is often called the \emph{invariance semantics} \sidecite[][Section 3.3]{Urban2015c}.
...
\begin{description}
  ...
  \item[\normalfont ($\plifstmt$)] Similarly to the forward reachability semantics for the conditional statement,\marginnote{
    \begin{align*}
      &\backwardreachabilitytracesemanticsnoparam \semanticsof{\plifstmt}\defsetofstates \DefeQ \\
      &\quad \setdef*{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\initialloc[\defstmt]}{\defenv} \in \backwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt} \defsetofstates \\ {} \land \bexpsemantics\defenv} \setjoin {} \\
      &\quad \setdef*{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\initialloc[\defstmt']}{\defenv} \in \backwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt'} \defsetofstates \\ {} \land \bexpsemanticsnoparam\semanticsof{\neg\defbexp}\defenv}
    \end{align*}
  ...
  \marginnote{
    \begin{align*}
      ...
      & \backwardreachabilitytracetransformer(X) \DefeQ \setdef*{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\initialloc[\defstmt]}{\defenv}\in\backwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt} X \\ {} \land \bexpsemantics\defenv} \SetjoiN {} \\
      &\quad\setdef@{\tracetuple{\bigloc}{\defenv}}{\tracetuple{\finalloc[\plwhilestmt]}{\defenv}\in\defsetofstates \\ {} \land \bexpsemanticsnoparam\semanticsof{\neg\defbexp}\defenv}
    \end{align*}
  }
...
\begin{marginfigure}
  ...
  \begin{align*}
    ...
    \\
    &\backwardreachabilitytracesemanticsnoparam \semanticsof{\pwhilestmt}\defsetofenvs \DefeQ \\ &\qquad
    \lfp^{\backwardreachabilitytracesubseteq} \backwardreachabilitytracetransformer
    \\
    &\quad \backwardreachabilitytracetransformer(X) \DefeQ \bexpsemanticsnoparam\semanticsof{\neg\defbexp}\defsetofenvs \SetjoiN {} \\ &\qquad \bexpsemantics(\backwardreachabilitytracesemanticsnoparam \semanticsof{\defstmt} X)
    \\
    &\backwardreachabilitytracesemanticsnoparam \semanticsof{\pcompstmt}\defsetofenvs \DefeQ \\
    &\quad \backwardreachabilitytracesemanticsnoparam\semanticsof{\defstmt}(\backwardreachabilitytracesemanticsnoparam\semanticsof{\defstmt'}\defsetofenvs)
    \\
...
which provide a sound and computable over-approximation of the concrete semantics.
  We introduce the concept of abstract domains with a simple example of the integer interval abstract domain \sidecite{Cousot1976,Hickey2001}, called \intervalname, \wrt{} the concrete domain of sets of integers, \ie, $\values = \Z$.
...
%
    ...
    \begin{definition}[Sound Operator]\labdef{sound-operator}
 ...
 \end{align*}
  ...
  \end{align*}
  \end{definition}
  Note that, \refdef{soundness-join-meet} is always satisfied for best abstractions operators, \cf{} \refdef*{best-operator-abstraction}[*-6].
  \newpage
...
\end{definition}
  ...
  \begin{theorem}
    ...
    The function $f \in X \to X$ is a monotonic operator and $g \in \abstractdomain \to \abstractdomain$ is a sound abstraction of $f$, \cf{} \refdef*{sound-operator}, the following iteration:
    \begin{align*}
...
\begin{definition}[Numerical Abstract Domain]\labdef{abstract-domain}
  ...
  \begin{itemize}
    ...
    \item sound join $\abstractdomainjoin\in\abstractdomain\times\abstractdomain\to\abstractdomain$ and meet $\abstractdomainmeet\in\abstractdomain\times\abstractdomain\to\abstractdomain$ operators, \cf{} \refdef{soundness-join-meet};
    ...
    \item a sound backward substitution $\abstractdomainsubstitute{\defvariable}{\defaexp}\in\abstractdomain\to\abstractdomain$ operator, \cf{} \refdef{soundness-substitution}; and
...
\begin{remark}[Relational Abstract Domains]
  The integer interval abstract domain is a numerical abstract domain since it satisfies \refdef{abstract-domain}.
...
\begin{description}
  \item[Signs \cite{Cousot1977}:] \phantomcite{Cousot1977} The sign domain collects constraints about the sign of the variables: positive, negative, zero, anything, or nothing. The domain is--potentially--the fastest available as it simply associates a sign to each variable. Unfortunately, it is rarely useful in practice.
  \item[Powerset \cite{Min_e2017}:] \phantomcite{Min_e2017} The powerset domain associates each variable with a finite set of possible values. This domain is quite precise but becomes quickly intractable as the number of possible values grows.
  \item[Excluded Powerset \cite{Monat2024}:] \phantomcite{Monat2024} The excluded powerset domain is a variant of the powerset domain that keeps track of the values that a variable definitely cannot take. This domain is able to compute disjunctions precisely but suffers from even higher scalability issues than the precedent.
  ...
  \item[Congruences \cite{Granger1991}:] \phantomcite{Granger1991} The congruence domain gathers constraints about the remainders of the variables when divided by a constant. This domain is particularly useful when dealing with modular arithmetic and pointer offsets.
\end{description}
...
\begin{description}
  \item[Linear Equalities \cite{Karr1976}:] \phantomcite{Karr1976} The linear equalities domain collects constraints about the linear relationships between variables.
  \item[Polyhedra \cite{Cousot1978}:] \phantomcite{Cousot1978} The polyhedra domain retrieves constraints about linear relationships between convex combinations of variables. Thanks to its expressiveness, the polyhedra domain is usually (depending on the application) one of the most precise domain available for numerical analyses.
  \item[Octagons \cite{Min_e2006a}:] \phantomcite{Min_e2006a} The octagons domain gathers constraints between variables of the form $x - y \le c$. One of the best trade-offs between precision and scalability among the relational domains.
\end{description}

Different abstract domains represent possibly complementary information about the program variables. Therefore, it is common to combine multiple abstract domains to leverage the strengths of each of them. A combination of abstract domains is known as the \emph{reduced product} domain \sidecite{Cousot1979a}. The reduced product domain is defined as the Cartesian product of the abstract domains where a single representation for the bottom element is enforced.
...
\end{itemize}
\end{definition}
\index{Abstract Domain!Reduced Product Domain}
...
\index{Forward Semantics!Abstract}
...
\begin{description}
  ...
  \end{align*}
  After each iteration, the $i$-th abstract element $x^i$ is widened with the result of the abstract transformer applied to it. The limit of the sequence can be reached in a finite amount of iteration, \cf{} \refdef{widening}.
  \marginnote{
    \begin{align*}
...
\begin{description}
  ...
  \marginnote{
    \begin{align*}
      &\backwardsemanticsnoparam \semanticsof{\passignstmt}\defabstractvalue \DefeQ \abstractdomainsubstitute{\defvariable}{\defaexp}\defabstractvalue
    \end{align*}
  }
  ...
  \item[\normalfont ($\pprogstmt$)]
    ...
    \begin{definition}[Backward Co-Reachability State Abstract Semantics]\labdef{backward-semantics}
