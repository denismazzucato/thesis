...
This chapter is based on work published at NASA Formal Methods Symposium (NFM) 2024 \cite{Mazzucato2024nfm}.
...
\begin{example}
  ...
  \begin{align*}
    ...
    \right\}
  \end{align*}
  As explained above, for any $\deftrace\in \tracesemanticsnoparam\semanticsof{\texttt{id}}$ we collect the set of input values that belong to traces $\deftrace'\in\tracesemanticsnoparam\semanticsof{\texttt{id}}$ that share the same output abstraction as $\deftrace$, \cf{} $I_{\deftrace}(\tracesemanticsnoparam\semanticsof{\texttt{id}})$.
  ...
  \begin{align*}
    ...
    \left\{
      ...
      \right\}
  \end{align*}
  In this case, the set of input values that can produce the outcome $0$ is $\{\tracetuple{\locinit}{\langle 0\rangle}, \tracetuple{\locinit}{\langle 1\rangle}, \tracetuple{\locinit}{\langle 2\rangle}\}$, for the outcome $1$ is $\{\tracetuple{\locinit}{\langle 1\rangle}, \tracetuple{\locinit}{\langle 2\rangle}\}$, and for the outcome $2$ is $\{\tracetuple{\locinit}{\langle 1\rangle}, \tracetuple{\locinit}{\langle 2\rangle}\}$.
 ...
 \end{lstlisting}
  ...
  \begin{align*}
    ...
    \left\{
      ...
      \right\}
  \end{align*}
  In this case, the set of input values that can produce the outcome $\true$ is $\{\tracetuple{\locinit}{\langle \true, \true\rangle}, \tracetuple{\locinit}{\langle \false, \true\rangle}\}$, for the outcome $\false$ is $\{\tracetuple{\locinit}{\langle \true, \false\rangle}, \tracetuple{\locinit}{\langle \false, \true\rangle}, \tracetuple{\locinit}{\langle \false, \false\rangle}\}$.
...
For instance, a program that may generate such a set of traces is the \refprog{zero-or-rand}.
\begin{marginlisting}
  \caption{Zero or random program.}
  \labprog{zero-or-rand}
  \vspace{\lineheight}
\begin{lstlisting}
zero_or_rand(x, y):
  if not y:
...
\begin{marginfigure}
  \begin{tikzpicture}
    ...
    \node (TT) at (0,1) {${\true\true}$};
    \node (TF) at (1,1) {${\true\false}$};
    \node (FT) at (2,1) {${\false\true}$};
    \node (FF) at (3,1) {${\false\false}$};

    \node (T) at (.5,0) {${\true}$};
    \node (F) at (2.5,0) {${\false}$};
    ...
    \draw[decorate,decoration={brace,amplitude=10pt,mirror,raise=5pt},yshift=0pt] (-0.5,1.5) -- (-0.5,-0.5) node [black,midway,xshift=-0.6cm] {$A$};
\end{tikzpicture}
\end{marginfigure}
...
For instance, we could use the backward semantics \refdef{?}\sidenote{\denis{This definition will be defined in \refch{abstract-interpretation}, and referenced on the side here.}} to compute $\backwardsemanticsnoparam(\bucket)$.
...
\begin{definition}[Concretization of the Measured Quantity]\labdef{concretization-quantitative}
  ...
  \begin{eqnarray*}
    ...
    & \setdef*{\defsetofdependencies \in \multiconcretization(\multisemantics)\buckets}{
      \impactwrapper(\defsetoftraces) \\
      \quad \ComparisoN \impactinstance(\multisemantics\buckets, \buckets)
    }
  \end{eqnarray*}
  where $\defsetoftraces\in\tracetype$ is the set of traces concretized from the dependencies $\defsetofdependencies$, \ie, $\defsetoftraces = \singletonelimination(\dependencyconcretization \circ \outputconcretization(\{\defsetofdependencies\}))$.
  \marginnote{The operator $\singletonelimination \in \setof A \to \setof A$ removes the set around a singleton set, \ie, $\singletonelimination(\{a\}) = a$.}
\end{definition}
...
Indeed, some subset of dependencies may not satisfy the property $\genbounded$.
...
\begin{example}\labexample{abstract-range}
  ...
  \begin{align*}
    ...
    \abstractdomaindistance(\bucket[\textup{low}] \abstractdomainjoin \bucket[\textup{mid\-/low}] \abstractdomainjoin \bucket[\textup{mid\-/high}]) \spacearound{&=} \abstractdomaindistance(\langle \texttt{risk} \mapsto [0, 2] \rangle) \spacearound{=} 2 \\
  \end{align*}
  Therefore, $\abstractrangename_{\{\texttt{speed}\}}(\presfrombuckets, \buckets) = 2$.
\end{example}

To prove that $\abstractrange$ is a sound implementation of $\range$, we require the classic soundness condition on the abstract operator $\abstractdomaindistance$, ensuring that the abstract size is always greater than the concrete one.
...
\begin{lemma}[$\abstractrange$ is a Sound Implementation of $\range$]\lablemma{abstractrange-is-sound}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \label{proof:ae} $\abstractdomaindistance$ is sound, \cf{} \refdef*{soundness-size}[*12];
  \end{enumerate}
...
\begin{proof}
  ...
  \begin{gather*}
    ...
    \distance(\setdef{\outputobs(\retrieveoutput{\deftrace})(\defoutputvariable)}{\deftrace\in\defsetoftraces\land\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \definput})
    \LE \\
  ...
  \begin{gather*}
    ...
    \distance(\setdef{\outputobs(\retrieveoutput{\deftrace})(\defoutputvariable)}{\deftrace\in\defsetoftraces\land\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \definput})
    \spacearound{=} \\
    \distance(\setdef{\outputobs(\retrieveoutput{\deftrace})(\defoutputvariable)}{\deftrace\in\overline{\defsetoftraces}\land\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \overline{\definput}})
  \end{gather*}
  ...
  We need to show that $\distance(\setdef{\outputobs(\retrieveoutput{\deftrace})(\defoutputvariable)}{\deftrace\in\overline{\defsetoftraces}\land\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \overline{\definput}}) \le \abstractdomaindistance(\overline{K})$.
  By contradiction, we assume that $\distance(\setdef{\outputobs(\retrieveoutput{\deftrace})(\defoutputvariable)}{\deftrace\in\overline{\defsetoftraces}\land\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \overline{\definput}}) > \abstractdomaindistance(\overline{K})$, meaning that the size of output abstraction over the variable $\defoutputvariable$ from states in $\overline{\defsetoftraces}$ is higher than the size of $\overline{K}$ in the abstract. Hence, either:
  \begin{enumerate}[label=(\alph*)]
...
\begin{definition}[\texorpdfstring{$\abstractqused$}{Abstract QUsed}]\labdef{abstract-qused}
  ...
  \begin{align*}
    \abstractqused(\presfrombuckets, \buckets) \DefeQ& \max_{j \le \numberofbuckets}\spacer
      \abstractdomaincount(\abstractdomainprojectothers(\inputspace) \setminus \abstractdomainprojectothers(\prefrombucket))
      \cdot
      \prod_{k \in \inputvariableswithoutw} \abstractdomaincount(\abstractdomainproject[\inputvariables \setminus \{k\}](\inputspace \setminus \abstractdomainbigjoin_{p \neq j}( \prefrombucket[p])))
  \end{align*}
