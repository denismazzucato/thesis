...
We note that the above notion is monotone in the amount of traces in $\defsetoftraces$.
...
%
  Here, instead of counting the distinct outcomes, we are interested in the difference between the extreme values of the outcomes. Hence, we apply the function $\size$: $\size(\{0,3\})=3$.
...
\begin{definition}[\texorpdfstring{$\abstractrange$}{Abstract Range}]\labdef{abstract-range}
  ...
  \begin{align*}
    \abstractrange(X^\natural, \buckets) \DefeQ& \max~\seTDef{\abstractdomainsize(K)}{K \in I} \\
    \text{where}~
