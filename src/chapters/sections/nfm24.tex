...
\begin{example}
  ...
  \begin{align*}
    ...
    &\qquad\LE \abstractrangename^\natural_{\{\texttt{angle}\}}(\multiconcretization(\multisemanticsnoparam)\buckets, \buckets) = 3
  \end{align*}
  \begin{align*}
...
\begin{lemma}[$\abstractqused$ is a Sound Implementation of $\qused$]\lablemma{abstractqused-is-sound}
  ...
  Whenever the following conditions hold:
  \begin{enumerate}[label=(\roman*)]
    \item \label{pp1} $\buckets$ covers the subset of potential outcomes, \cf{} \refdef*{covering},
    \item \label{pp2} $\buckets$ is compatible with $\outputobs$, \cf{} \refdef*{compatibility},
    \item \label{pp3} $\abstractdomainproject$ is sound, \cf{} \refdef*{soundness-project}, and
    \item \label{pp4} $\abstractdomaincount$ is sound, \cf{} \refdef{soundness-count};
  \end{enumerate}
