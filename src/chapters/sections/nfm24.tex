...
Later this thesis, we will apply our quantitative framework in the context of neural networks and timing side-channel attacks.
...
\marginnote{
  \begin{tikzpicture}[scale=0.9]
    ...
    \draw[->] (FT) -> (T);
    \draw[->] (FF) -> (F);
    \draw[decorate,decoration={brace,amplitude=8pt,mirror,raise=4pt},yshift=0pt,xshift=0pt] (-0.8,2.2) -- (-0.8,-0.2) node [black,midway,xshift=-0.6cm,rotate=90] {$\tracesemanticsnoparam\semanticsof{\texttt{id2}}$};
\end{tikzpicture}
}
\begin{example}
  ...
  \begin{align*}
    ...
    \right\}
  \end{align*}
  as $\qusedname_{\{\texttt{x}\}}(\tracesemanticsnoparam\semanticsof{\texttt{id2}})$ is 0 in such case (\cf{} \refexample{qused-not-monotonic}).
  Note that, there is a contradiction as the abstract impact $\impactinstance(\multisemanticsnoparam\semanticsof{\texttt{id2}}, \buckets)$ is assumed to be $1$ and should be smaller that the concrete one, which is $0$.
...
\begin{theorem}[Soundness] \labthm{soundness}
  ...
  Whenever,
  \begin{enumerate}[label=(\roman*)]
    \item \label{p:first} $\backwardsemanticsnoparam$ is sound with respect to $\outputsemanticsnoparam$, \cf{} \refdef*{sound-over-approximation}[*-5],
...
Next, we show \nrefthm{soundness} instantiated with the abstract implementation $\abstractoutcomes$.
...
These intersections represent concrete input configurations where variations on the values of $\definputvariables$ \emph{may} lead to changes of program outcome, from a bucket to another.
All the buckets of corresponding intersections are joined together and the projected away all but the output variables (\cf{} $\abstractdomainproject[\inputvariables \setminus \defoutputvariables]$) to find the maximum range of the extreme values of the buckets.
...
Next, we show \nrefthm{soundness} instantiated with the abstract implementation $\abstractrange$.
...
\end{enumerate}
  the following implication holds:
  \begin{align*}
    \abstractrange(\multisemantics\buckets, \buckets) = \defbound' \land \defbound' \le \defbound \ImplieS \defprogram \satisfies \BOUNDEDOUTCOMESLE
  \end{align*}
\end{theorem}
\begin{proof}
...
We define $\abstractqused$ as the number of input values that are missing from perturbation to the input variables $\definputvariables$ to reach the same outcomes. We assume the abstract domain $\abstractdomain$ is equipped with an abstract counting operator $\abstractdomaincount\in\abstractdomain\to\valuesposplus$ that returns the number of possible values of the given abstract element, \eg, $\abstractdomaincount(\langle \defvariable \mapsto [2, 4]\rangle) = 3$.
As well as an abstract complement operator $\abstractdomaincomplement_{\defabstractvalue}\in\abstractdomain\to\abstractdomain$ that returns the complement of the given abstract element \wrt{} a given $\defabstractvalue$ which we assume to represent the whole space, \eg, assuming $\abstractdomaincomplement_{\langle \defvariable \mapsto \{0, 1, 2, 3\}\rangle}(\langle \defvariable \mapsto \{2, 3\}\rangle) = \langle \defvariable \mapsto \{0, 1\} \rangle$.
In general, we use $\abstractdomaintop$ for the abstract element representing the whole space.
...
Next, we show \nrefthm{soundness} instantiated with the abstract implementation $\abstractqused$.
...
Given the connection between \emph{qualitative} input usage and information flow analyses \sidecite{Urban2018}, to design a quantitative input usage analysis that fits our purposes, it may come natural to use \emph{quantitative} information flow \sidecite{Denning1982,Gray1991}.
...
To address the previous issue, as a second attempt we exploit low and high labels for input variables, where the former are considered as public, available to the attacker, and the latter as secret.
...
Other works include \sidetextcite{Chothia2014}, which proposed a statistical approach to quantify information leakage for Java programs. \sidetextcite{Phan2012} and \sidetextcite{Saha2023} employed symbolic execution techniques and model counting to obtain sound bounds on the entropy of programs.
...
% Input data usage and non-interference relates also with \textit{Data Leakage}~\cite[Chapter~4]{Brownlee2020}.
...
% The series of papers developed by Subotić et al.~\cite{Drobnjaković2022,Subotić2021,Subotić2022} focuses on verifying data leakage within code notebooks and external libraries such as NumPy or pandas, bringing new challenges in the topic of formal verification.
% Within our framework, we could incorporate an adaptation of their work to encompass a notion of quantitative data leakage.
