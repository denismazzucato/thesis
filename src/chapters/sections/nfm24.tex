...
To this end, these definitions employ the concept of output abstraction, \cf{} \refdef*{output-observer}[*-4].
...
\begin{proof}
  ...
  \begin{eqnarray*}
    \lefteqn{\outputvaluesabstraction(\dependencyabstraction(\boundedoutcomes)) =} \\
    & \setdef{
...
%
  Here, instead of counting the distinct outcomes, we are interested in the difference between the extreme values of the outcomes. Hence, we apply the function $\length$: $\length(\{0,3\})=3$.
...
\begin{proof}
  ...
  \begin{align*}
    & \outputvaluesabstraction(\dependencyabstraction(\boundedrange)) = \\
    &\quad
    \setdef{
...
\begin{proof}
  ...
  \begin{eqnarray*}
    \lefteqn{\outputvaluesabstraction(\dependencyabstraction(\boundedqused)) =} \\
    &
    \setdef{
      \defsetofdependencies \in \outputtype
    }{
...
Since the abstract semantics may discover more dependencies than the concrete ones, the concretization returns all the non-empty combination of dependencies, \cf{} the power set operator $\wp_{+}$.
...
In the context of the interval domain, where each input variable is related to a possibly unbounded lower and upper bound, $\abstractdomainprojecti(\langle\definputvariable \mapsto [1, 3], j \mapsto [2, 4]\rangle) = \langle \definputvariable \mapsto [-\infty, +\infty], j \mapsto [2, 4] \rangle$
  ...
  Often, whenever a variable maps to $\abstractdomaintop$, or equivalently $[-\infty, +\infty]$, we hide it from the abstract element, \eg{} $\langle \definputvariable \mapsto [-\infty, +\infty], j \mapsto [2, 4] \rangle = \langle j \mapsto [2, 4] \rangle$.
\end{example}
%
...
\begin{example}\labexample{abstract-outcomes}
  ...
  \begin{align*}
    \backwardsemanticsnoparam(\bucket[\textup{low}]) \spacearound{&=} X^\natural_{\textup{low}} \spacearound{=} \langle \texttt{angle} \mapsto 1, \texttt{speed} \mapsto [1, 3] \rangle \\
    \backwardsemanticsnoparam(\bucket[\textup{mid\-/low}]) \spacearound{&=} X^\natural_{\textup{mid\-/low}} \spacearound{=} \langle \texttt{angle} \mapsto 1, \texttt{speed} \mapsto [2, 3] \rangle \\
    \backwardsemanticsnoparam(\bucket[\textup{mid\-/high}]) \spacearound{&=} X^\natural_{\textup{mid\-/high}} \spacearound{=} \langle \texttt{angle} \mapsto 1, \texttt{speed} \mapsto [2, 3] \rangle \\
    \backwardsemanticsnoparam(\bucket[\textup{high}]) \spacearound{&=} X^\natural_{\textup{high}} \spacearound{=} \langle \texttt{angle} \mapsto -4, \texttt{speed} \mapsto [1, 3] \rangle \\
  \end{align*}
  ...
  \begin{align*}
    \texttt{Proj}^\natural_{\{\texttt{angle}\}}(X^\natural_{\textup{low}}) \spacearound{&=} \langle \texttt{speed} \mapsto [1, 3] \rangle \\
    \texttt{Proj}^\natural_{\{\texttt{angle}\}}(X^\natural_{\textup{mid\-/low}}) \spacearound{&=} \langle \texttt{speed} \mapsto [2, 3] \rangle \\
    \texttt{Proj}^\natural_{\{\texttt{angle}\}}(X^\natural_{\textup{mid\-/high}}) \spacearound{&=} \langle \texttt{speed} \mapsto [2, 3] \rangle \\
    \texttt{Proj}^\natural_{\{\texttt{angle}\}}(X^\natural_{\textup{high}}) \spacearound{&=} \langle \texttt{speed} \mapsto [1, 3] \rangle \\
  \end{align*}
...
\end{eqnarray*}
  ...
  Therefore, $\abstractoutcomes(X^\natural, \buckets) = 4$, meaning that variations of the input variable \texttt{angle} may lead to changes of program outcome from four different buckets.
  On the other hand, projecting away the variable \texttt{speed} from the abstract intervals results in:
  \begin{align*}
    \texttt{Proj}^\natural_{\{\texttt{speed}\}}(X^\natural_{\textup{low}}) \spacearound{&=} \langle \texttt{angle} \mapsto 1 \rangle \\
    \texttt{Proj}^\natural_{\{\texttt{speed}\}}(X^\natural_{\textup{mid\-/low}}) \spacearound{&=} \langle \texttt{angle} \mapsto 1 \rangle \\
    \texttt{Proj}^\natural_{\{\texttt{speed}\}}(X^\natural_{\textup{mid\-/high}}) \spacearound{&=} \langle \texttt{angle} \mapsto 1 \rangle \\
    \texttt{Proj}^\natural_{\{\texttt{speed}\}}(X^\natural_{\textup{high}}) \spacearound{&=} \langle \texttt{angle} \mapsto -4 \rangle \\
  \end{align*}
  ...
  & \left\{
    ...
    \right\}
  \end{eqnarray*}
  The maximum clique is of size $3$, \cf{} \cardinalitynospaces{\textup{low}, \textup{mid\-/low}, \textup{mid\-/high}}, hence $\abstractoutcomes(X^\natural, \buckets) = 3$. This means that variations of the input variable \texttt{speed} may lead to changes of program outcome in three different buckets.
\end{example}
...
\begin{proof}
  ...
  \begin{gather*}
    ...
    \cardinality{\setdef{\outputobs(\retrieveoutput{\defstate})}{\inputoutputtuple\defstate\in\dependencysemanticsnoparam\land\retrieveinput\defstate \stateeq{\inputvariableswithoutw} \definputvariables}} \\
    \le \\
...
\begin{example}
  ...
  \begin{align*}
    ...
    &\qquad\LE \abstractoutcomesname^\natural_{\{\texttt{speed}\}}(X^\natural, \buckets) = 3 \\
  \end{align*}
  as expected by \reflemma{abstractoutcomes-is-sound}.
\end{example}
...
\begin{example}
  In the context of the interval domain, where each input variable is related to a possibly unbounded lower and upper bound, $\abstractdomainlength(\langle \defvariable \mapsto [2, 4]\rangle) = 2$.
  On the other hand, whenever the input abstract element is unbounded, the size is $+\infty$, \eg{} $\abstractdomainlength(\langle \defvariable \mapsto [0, +\infty]\rangle) = +\infty$.
\end{example}
...
\begin{example}\labexample{abstract-range}
  ...
  \begin{align*}
    \abstractdomainlength(\bucket[\textup{low}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto 0 \rangle) \spacearound{=} 0 \\
    \abstractdomainlength(\bucket[\textup{mid\-/low}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto 1 \rangle) \spacearound{=} 0 \\
    \abstractdomainlength(\bucket[\textup{mid\-/high}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto 2 \rangle) \spacearound{=} 0 \\
    \abstractdomainlength(\bucket[\textup{high}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto 3 \rangle) \spacearound{=} 0 \\
    \abstractdomainlength(\bucket[\textup{low}] \abstractdomainjoin \bucket[\textup{mid\-/low}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto [0, 1] \rangle) \spacearound{=} 1 \\
    \abstractdomainlength(\bucket[\textup{low}] \abstractdomainjoin \bucket[\textup{mid\-/high}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto [0, 2] \rangle) \spacearound{=} 2 \\
    \abstractdomainlength(\bucket[\textup{mid\-/low}] \abstractdomainjoin \bucket[\textup{mid\-/high}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto [1, 2] \rangle) \spacearound{=} 1 \\
    \abstractdomainlength(\bucket[\textup{low}] \abstractdomainjoin \bucket[\textup{mid\-/low}] \abstractdomainjoin \bucket[\textup{mid\-/high}]) \spacearound{&=} \abstractdomainlength(\langle \texttt{risk} \mapsto [0, 2] \rangle) \spacearound{=} 2 \\
  \end{align*}
  Therefore, $\abstractrangename^\natural_{\{\texttt{speed}\}}(X^\natural, \buckets) = 2$.
\end{example}
...
\begin{definition}[\texorpdfstring{$\abstractqused$}{Abstract QUsed}]\labdef{abstract-qused}
  ...
  \begin{align*}
    \abstractqused(X^\natural, \buckets) \DefeQ& \cardinality{\inputvaluesof{\state}} -\min~  \seTDef{\abstractdomaincount(\abstractdomainprojectothers(X^\natural_j))}{j \le n}
  \end{align*}
\end{definition}
...
\begin{example}
  ...
  \begin{align*}
    ...
    &\qquad\GE \abstractqusedname^\natural_{\{\texttt{speed}\}}(X^\natural, \buckets) = 1
  \end{align*}
  as expected by \reflemma{abstractqused-is-sound}.
\end{example}
