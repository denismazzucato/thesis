...
The soundness of the approach leverages two elements: (1) the backward abstract semantics (\cf{} \refdef{?}\sidenote{This definition will be defined in \refch{abstract-interpretation}}), and (2) sound and computable implementations of the quantitative measures \outcomesname{}, \rangename{}, and \qusedname{}, written as $\abstractoutcomes$, $\abstractrange$, and $\abstractqused$ respectively.
...
We define the concretization of the abstract implementation $\impactwrappername$ as $\boundconcretization \in \valuesinf \to \collectingtype$. Such function should return the set of trace semantics that yield an impact lower (or bigger) than the bound $\defbound$.
...
\begin{definition}[Concretization of the Measured Quantity]\labdef{concretization-quantitative}
  ...
  \begin{align*}
    \boundconcretization(\defbound) \DefeQ
    \setdef{\defsetoftraces \in \collectingtype}{
      \impactwrapper(\defsetoftraces) \comparison \defbound
    }
  \end{align*}
\end{definition}

\marginnote{It holds an equivalence with the $\defbound$-bounded impact property:
\[
  \boundconcretization(\defbound) = \genbounded
\]}

We note that the concretization $\boundconcretization$ is monotonic in the bound $\defbound$.
