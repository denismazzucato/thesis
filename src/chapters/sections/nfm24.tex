...
\marginnote{\denis{how to make the command marginnote remove the little space and be totally anonymous in the text? see the space in front of Definition 4.1.1}}
...
\end{eqnarray*}
  ...
  & \left\{
    ...
    \right\}
  \end{eqnarray*}
  The maximum clique is of size $3$, \cf{} \cardinalitynospaces{\{\textup{low}, \textup{mid\-/low}, \textup{mid\-/high}\}}, hence $\abstractoutcomes(\presfrombuckets, \buckets) = 3$. This means that variations of the input variable \texttt{speed} may lead to changes of program outcome in three different buckets.
\end{example}
...
\end{lemma}
\begin{proof}
  From \ref{proof:a} and \ref{proof:b2}, \reflemma{sound-approximation-covering} demonstrates that $\outputsemantics \subseteq \multiconcretization(\multisemantics)\buckets$.
  Since $\outcomes$ and $\dependencyconcretization \circ \outputconcretization$ are monotone (respectively by \reflemma{outcomes-monotonic} and both \refthm{collecting-dependency-galois-connection}, \refthm{dependency-output-galois-connection}) we obtain that $\outcomes(\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)) \le \outcomes(\dependencyconcretization \circ \outputconcretization(\multiconcretization(\multisemanticsnoparam)\buckets))$.
