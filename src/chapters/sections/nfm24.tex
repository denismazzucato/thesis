...
\end{eqnarray*}
  ...
  Therefore, $\abstractoutcomesname^\natural_{\{\texttt{angle}\}}(\presfrombuckets, \buckets) = 4$, meaning that variations of the input variable \texttt{angle} may lead to changes of program outcome from four different buckets.
  ...
  & \left\{
    ...
    \right\}
  \end{eqnarray*}
  The maximum clique is of size $3$, \cf{} \cardinalitynospaces{\{\textup{low}, \textup{mid\-/low}, \textup{mid\-/high}\}}, hence $\abstractoutcomesname^\natural_{\{\texttt{speed}\}}(\presfrombuckets, \buckets) = 3$. This means that variations of the input variable \texttt{speed} may lead to changes of program outcome in three different buckets.
\end{example}
...
\begin{proof}
  ...
  \end{gather*}
  Let $\outcomes(\defsetoftraces)=\defbound$ be any of the quantity measured by the concrete $\outcomes$.
  By contradiction, we assume that the higher cardinality set from $\intersectallfunction((\abstractdomainproject(\prefrombucket))_{j\le\numberofbuckets})$ is lower than $\defbound$.
  Such a set contains all the indices that intersect after projection.

  \denis{To finish, and coherently adapt side references once done.}
  % First we notice that $\tracesemanticsnoparam\in\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)$ as by definition of the concretization functions and that for any $\defsetoftraces\in\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)$ it holds that $\outcomes(\defsetoftraces) \le \outcomes(\tracesemanticsnoparam)$, as the output-abstraction semantics $\outputsemanticsnoparam$ is sound and complete.
  % From \ref{proof:a} and \ref{proof:b2}, \reflemma*{sound-approximation-covering} demonstrates that $\outputsemanticsnoparam \subseteq \multiconcretization(\multisemanticsnoparam)\buckets$.
  % Since $\dependencyconcretization \circ \outputconcretization$ are monotone, \cf{} \refthm{collecting-dependency-galois-connection} and \refthm{dependency-output-galois-connection}, we obtain that
  % $\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam) \subseteq \dependencyconcretization \circ \outputconcretization(\multiconcretization(\multisemanticsnoparam)\buckets)$.
  % Next, from the monotonicity of $\outcomes$, \cf{} \reflemma*{outcomes-monotonic}, we have that for any $\defsetoftraces\in\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)$ it exists $\defsetoftraces'\in\dependencyconcretization \circ \outputconcretization(\multiconcretization(\multisemanticsnoparam)\buckets)$ such that
  % $\outcomes(\defsetoftraces) \le \outcomes(\defsetoftraces')$.
  % We notice that $\tracesemanticsnoparam\in\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)$ as by definition of the concretization functions.
%   Furthermore, for any $\defsetoftraces\in\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)$ it holds that $\outcomes(\defsetoftraces) \le \outcomes(\tracesemanticsnoparam)$, as the output-abstraction semantics $\outputsemanticsnoparam$ is sound and complete.
%   By definitions of $\abstractoutcomes$, \cf{} \refdef{abstract-outcomes}, and $\outcomes$, \cf{} \refdef{outcomes}, for any $\defsetoftraces\in\dependencyconcretization \circ \outputconcretization(\outputsemanticsnoparam)$, we need to show that
%   \begin{gather*}
%     \outcomes(\defsetoftraces) = \sup_{\definput\in\reducedstate}
%     \cardinality{\setdef{\outputobs(\retrieveoutput{\deftrace})}{\deftrace\in\defsetoftraces\land\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \definput}} \\
%     \le \\
...
% %
%   First, from \ref{proof:b2} and \ref{proof:b1}, \reflemma*{outcomes-upper-bound} demonstrates that $\outcomes(\tracesemanticsnoparam)$ is limited by the number of buckets $n$.
%   Notably, $\outcomes$ cannot be unbounded, but it has to be a finite number, at most $n$. Thus, it exists an initial state $\overline\defstate\in\reducedstate$ such that $\outcomes(\outputsemanticsnoparam) = \cardinalitynospaces{\defsetoftraces_{\overline\defstate}}$, where $\defsetoftraces_{\overline\defstate} = \setdef{\outputobs(\retrieveoutput{\defstate})}{\inputoutputtuple\defstate\in\outputsemanticsnoparam\land\retrieveinput\defstate \stateeq{\inputvariableswithoutw} \overline\defstate}$.
%   We conclude in case this cardinality is $0$ as any value returned by $\abstractoutcomes$ would be greater or equal. In the other case, by covering (\cf{} \ref{proof:b2}), for all dependencies in $\defsetoftraces_{\overline\defstate}$
%   it exists a bucket $\bucket$ such that $\retrieveoutput{\defstate'}\in\abstractdomainconcretization(\bucket)$.

%   Furthermore, by compatibility (\cf{} \ref{proof:b1}), for any pair $\inputoutputtuple\defstate, \inputoutputtuple{\defstate'}\in\defsetoftraces_{\overline\defstate}$ leading to two different outcomes $\outputobs(\retrieveoutput{\defstate}) \neq \outputobs(\retrieveoutput{\defstate'})$, we have two different buckets $\bucket, \bucket[p]$ such that $\retrieveoutput{\defstate}\in\abstractdomainconcretization(\bucket)$ and $\retrieveoutput{\defstate'}\in\abstractdomainconcretization(\bucket[p])$.
%   Note that, by definition of $\defsetoftraces_{\overline\defstate}$ it holds that $\retrieveinput\defstate \stateeq{\inputvariableswithoutw} \overline{\defstate} \stateeq{\inputvariableswithoutw} \retrieveinput{\defstate'}$, by transitivity $\retrieveinput\defstate \stateeq{\inputvariableswithoutw} \retrieveinput{\defstate'}$.
...
\begin{example}
  ...
  \marginnote{
    ...
    \end{align*}}
  As expected by \refthm{soundness}, the bound computed by the abstract implementation $\abstractoutcomes$ is always greater or equal to the concrete one:
  \begin{align*}
  ...
  \end{align*}
  where $\landingprogram$ is the program of the landing alarm system, \cf{} \refprog*{landing-alarm-system}[*-7].
\end{example}
...
\begin{lemma}[$\abstractrange$ is a Sound Implementation of $\range$]\lablemma{abstractrange-is-sound}
  ...
  Whenever the following conditions hold:
  \begin{enumerate}[label=(\roman*)]
    \item \label{proof:ra} $\backwardsemanticsnoparam$ is sound with respect to $\outputsemanticsnoparam$, \cf{} \refdef{sound-over-approximation},
    \item \label{proof:rb} $\buckets$ covers the subset of potential outcomes, \cf{} \refdef{covering},
    \item \label{proof:rc} $\buckets$ is compatible with $\outputobs$, \cf{} \refdef{compatibility},
    \item \label{proof:rd} $\abstractdomainproject$ is sound, \cf{} \refdef{soundness-project}, and
    \item \label{proof:re} $\abstractdomainsize$ is sound, \cf{} \refdef{soundness-size};
  \end{enumerate}
...
\begin{example}
  ...
  \begin{align*}
    ...
    &\qquad\LE \abstractrangename^\natural_{\{\texttt{speed}\}}(\presfrombuckets, \buckets) = 2
  \end{align*}
  as expected by \reflemma{abstractrange-is-sound}.
\end{example}
...
\begin{example}\labexample{qused-sound}
...
\begin{example}
  ...
  \end{align*}
  as expected by \refthm{soundness}.
\end{example}
