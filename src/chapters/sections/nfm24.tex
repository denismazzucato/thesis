...
\end{definition}
\index{$k$-Bounded Impact Property}
...
\end{marginfigure}
  Originally introduced by \sidetextcite[*-8]{Giacobazzi2018}, the output observer $\outputobs$ should deal with any possible abstraction an end-user of the framework might want to use.
...
\begin{definition}[\outcomesname]\labdef{outcomes}
  ...
  \end{align*}
\end{definition}
\index{Impact Quantifier!$ \outcomes $}
...
\index{Property Validation!$ \outcomes $}
...
\siderefbox{def}{abstraction-dependency-semantics}
\siderefbox{def}{abstraction-output-abstraction-semantics}
\index{Property Validation!$ \range $}
...
\siderefbox{def}{abstraction-dependency-semantics}
\siderefbox{def}{abstraction-output-abstraction-semantics}
\index{Property Validation!$ \qused $}
...
\end{definition}
\index{Impact Quantifier!Abstract Implementation!$ \abstractoutcomes $}
...
\begin{lemma}[$\outcomes$ Upper Bound]\lablemma{outcomes-upper-bound}
  When the $\numberofbuckets$ output buckets $\buckets\in\vectorbuckets$ are compatible, \cf{} \refdef{compatibility}, and cover the subset of potential outcomes, \cf{} \refdef*{covering}[*-4], it holds that
...
\refdef@{outcomes}[*-4]
\index{Sound Implementation!\abstractoutcomes}
...
\index{Soundness!\abstractoutcomes}
...
\index{Impact Quantifier!Abstract Implementation!$ \abstractrange $}
...
\index{Sound Implementation!\abstractrange}
\begin{lemma}[$\abstractrange$ is a Sound Implementation of $\range$]\lablemma{abstractrange-is-sound}
  Let $\definputvariables\in\setof\inputvariables$ be the input variable of interest, $\abstractdomain$ the abstract domain, $\backwardsemanticsnoparam$ the family of semantics, and $\buckets\in\vectorbuckets$ the starting output buckets.
  Whenever the following conditions hold:
  \begin{enumerate}[label=(\roman*)]
    % \item \label{proof:a} $\backwardsemanticsnoparam$ is sound with respect to $\outputsemanticsnoparam$, \cf{} \refdef*{sound-over-approximation},
    \item \label{proof:ab} $\buckets$ covers the subset of potential outcomes, \cf{} \refdef*{covering}[*-10],
    \item \label{proof:ac} $\buckets$ is compatible with $\outputobs$, \cf{} \refdef*{compatibility}[*-5],
    \item \label{proof:ad} $\abstractdomainproject[]$ is sound, \cf{} \refdef*{soundness-project}[*0], and
    \item \label{proof:ae} $\abstractdomaindistance$ is sound, \cf{} \refdef*{soundness-size}[*5];
  \end{enumerate}
...
\begin{theorem}[Soundness of $\boundedrangele$] \labthm{soundness-boundedrangele}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item $\backwardsemanticsnoparam$ is sound with respect to $\outputsemanticsnoparam$, \cf{} \refdef*{sound-over-approximation}[*-5], and
    \item $\buckets$ covers the subset of potential outcomes, \cf{} \refdef*{covering}[*-1],
\end{enumerate}
  the following implication holds:
  \begin{align*}
...
\begin{definition}[\texorpdfstring{$\abstractqused$}{Abstract QUsed}]\labdef{abstract-qused}
  ...
  \end{align*}
\end{definition}\index{Impact Quantifier!Abstract Implementation!$ \abstractqused $}
...
Hence, the next result shows that the abstract impact $\abstractqusedname$ is a sound over-approximation of the concrete impact $\qusedname$, \cf{} \refdef*{qused}[*-3] \wrt{} the $\ge$ operator.

\index{Sound Implementation!\abstractqused}
...
The study of quantitative properties was inspired by \sidetextcite{Barowy2014}, where an empirical analysis explores the use of data for debugging spreadsheet applications through stochastic sampling.
...
Quantitative input data usage, which evolves from its qualitative counterpart, is related to work on abstract non-interference \sidecite{Giacobazzi2018}.
...
Given the connection between \emph{qualitative} input usage and information flow analysis \sidecite[][Section 8]{Urban2018}, to design a quantitative input usage analysis that fits our purposes, it may come natural to look into \emph{quantitative} information flow, formerly introduced by \sidetextcite{Denning1982,Gray1991}.
...
This similarity between entropy and our notion of impact is even more evident
...
Next, we show three successive attempts of adaptation of entropy measures to our needs.

\newcommand{\shannonentropy}{H}
\newcommand{\probability}{E_\defprogram}
First of all, note that Shannon-entropy computes the average uncertainty of input values based on observations of the program's outcomes, while min-entropy, defined as:
...
As a first attempt, we can consider min-entropy as it is closer to our need of discovering the worst-case impact, \ie, the case in which a variable contributes the most.
...
To address the previous issue, as a second attempt we can exploit low and high labels for input variables, where the former are considered as public, available to the attacker, and the latter as secret \sidecite{K_opf2010}.
...
Computing $\minentropy(\landingprogram^{\texttt{angle}})$ and $\minentropy(\landingprogram^{\texttt{speed}})$ yields 0 on both because all equivalence classes consist of singletons, meaning the number of inputs equals the number of outputs. Thus, there is no uncertainty in the value of ${\texttt{angle}}$ given outputs of $\landingprogram^{\texttt{angle}}$, or in the value of ${\texttt{speed}}$ given outputs of $\landingprogram^{\texttt{speed}}$.
...
This is the first notion of impact derived from min-entropy that is capable of discriminating the contribution of each input variable on the program computation, exploiting the number of reachable outcomes from variations of the value of the input variable under consideration.
Indeed, $\outcomesentropy(\defprogram^\definputvariables)=\outcomes(\defprogram)$ holds for a generic program $\defprogram$.

Overall, entropy measures and the approach proposed by \sidetextcite{K_opf2013,K_opf2010} can be adapted to our needs. Nevertheless, their analysis grows exponentially with the number of low variables, which in our adaptation corresponds to the number of inputs, minus one. To address this limitation, in this chapter we leveraged an over-approximation of input-output observations of the program, focusing solely on the low variables. By doing so, we obtain the set of input configurations that lead to the same output value by variation on the value of high variables.
...
\sidetextcite{McCamant2008} retrieves the channel capacity, which represents the worst-case potential for information leakage.
Based on the notion of an attacker's beliefs \sidecite{Clarkson2009}, Mardziel et al. \sidecite{Mardziel2011,Mardziel2013} originally defined the property of knowledge threshold security and \sidetextcite{Besson2014} proposed a flexible mechanism based on randomisation to enforce it.
...
Other recent developments include \sidetextcite{Zhang2022}, who presented a calculus based on strongest-postconditions style for quantitative reasoning about information flow, and \sidetextcite{Henzinger2023}, who generalized the hierarchy of safety and liveness properties to quantitative safety and liveness.
...
A series of papers by \sidetextcite{Drobnjakovic2024,Subotic2022a,Subotic2022b} focus on verifying data leakage within data science notebooks and external libraries such as NumPy or pandas, presenting new challenges in formal verification.
...
That said, using the terminology of \textcite{Clarkson2010}, our $\defbound$-bounded impact property is a hyperproperty.
...
\sidetextcite{Schr_oer2023} introduced a deductive framework for expectation-based reasoning about probabilistic programs.
...
Symbolic execution techniques have been adapted for probabilistic programs, enabling a detailed exploration of program paths under various stochastic scenarios \sidecite{Sankaranarayanan2013,Beutner2022}, and probabilistic model checking has been used to verify temporal and quantitative properties of probabilistic programs \sidecite{Hensel2022,Winkler2023}.
Static analysis is also used to verify inference algorithm's correctness and assumptions \sidecite{Lee2020}, and to even improve the gradient estimator \sidecite{Lee2023}.
