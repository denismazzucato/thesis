...
\begin{example}\labexample{output-observer-landing}
  Consider the \refprog*{landing-alarm-system}[*-6] for a landing alarm system with program states $\state=\setdef{\langle a, b, c, d \rangle}{a\in\{-4,1\}\land b\in\{1,2,3\}\land c\in\N \land d\in\{0,1,2,3\}}$, where $a$ is the value of $\text{angle}$, $b$ of $\texttt{speed}$, $c$ of $\texttt{landing\_coeff}$, and $d$ of $\texttt{risk}$.
...
The work presented in this chapter stems from \sidetextcite{Urban2018}, proposing a unifying view on existing information flow analyses based on abstract interpretation. In this section, we compare our work with the existing literature on non-interference as the foundational property for information flow analysis, entropy measures of quantitative information flow, and probabilistic programming languages.
...
As quantitative input data usage arises from the qualitative counterpart, this chapter has also been inspired by the work on abstract non-interference \sidecite{Giacobazzi2018}. In particular, \sidetextcite{Giacobazzi2004} introduced a domain-theoretic characterization of the most precise attacker which cannot violate abstract non-interference.
Later on, \sidetextcite{Mastroeni2013} instantiated various policies obtaining the different notions of abstract non-interference for language-based security.
Such work pavemented the way for the development of the unifying framework for weakening the non-interference property, into abstract non-interference \cite{Giacobazzi2018}. More recently, \sidetextcite{Mastroeni2023} explored the connection between domain completeness and narrow abstract non-interference, where domain transformers can be characterized using monotonicity, in the weak framework of abstract interpretation. Notably, in our work, from the abstraction of non-interference to abstract non-interference we adopted the concept of state abstractions, \cf{} \nrefdef{output-observer}.
...
%formerly introduced by the pioneering work of \sidetextcite{Denning1982} and \sidetextcite{Gray1991}.
...
% In particular, \textcolor{red}{we focus on the work proposed by \sidetextcite{K_opf2013}} \marginpar{\textcolor{red}{why this work and not others? we should justify}} which quantifies an upper bound of the entropy of a program's input variables by computing an over-approximation of the set of input-output observations, sometimes called equivalence classes. They employ Shannon entropy, min-entropy, and other entropies through the enumeration of these equivalence classes and their respective sizes. The equivalence classes are partitions of the input space in which two input assignments belong to the same class whenever the program produces an equivalent output.
...
% Various static analyses \sidecite{Clark2007,Malacaria2007} derived from information theory. Entropy measures, such as \textit{Shannon}- or \textit{min}-entropy, are employed to define average or worst-case bounds on the level of difficulty faced by an attacker in discovering a secret.
% These measures are commonly utilized to defend against security threats, \eg, side-channel attacks~\cite{Malacaria2018}.

% In particular, the work of \sidetextcite{K_opf2013} quantifies an upper bound of the entropy of a program's input variables by computing an over-approximation of the set of input-output observations, sometimes called equivalence classes. This framework quantifies Shannon entropy, min-entropy, and other entropies through the enumeration of these equivalence classes and their respective sizes. The equivalence classes are partitions of the input space in which two input assignments belong to the same class whenever the program produces an equivalent output.
% For example, the equivalence classes of the program of \Figure{program}, denoted as $\defprogram$, with output variable \z, are defined as $\entropypartitions=\setdef{\setdef{\tuple{x}{y}}{\defprogram(\tuple{x}{y})=\defoutput}}{\defoutput\in\{0, 1, 2, 3\}}
% =\{\{\tuple{1}{1}\}, \{\tuple{1}{2}\}, \{\tuple{1}{3}\}, \{\tuple{-4}{1}, \tuple{-4}{2}, \tuple{-4}{3}\}\}$.
...
% Given the set of all input values $\exampleinput$, the Shannon-entropy of a program is:
% \begin{align}
%   \labeq{shannon-entropy}
%   \shannonentropy(\defprogram) \DefeQ
%     \frac{1}{\cardinalitynospaces{\exampleinput}} \sum_{B\in\entropypartitions} \cardinalitynospaces{B} \log_2 \cardinalitynospaces{B}
% \end{align}
...
%
% We define $\landingprogram^{\texttt{angle}}_y(x)\defeq\landingprogram(x, y)$ as the program where {\texttt{angle}} is high and {\texttt{speed}} is low with value $y$.
%
...
% Effectively, this is the first notion of impact capable of discriminating the contribution of each input variable on the program computation, exploiting the number of reachable outcomes from variations of the value of the input variable under consideration.
...
% Our research draws inspiration from~\cite{Barowy2014} where an empirical analysis explores the usage of data for debugging spreadsheet applications through stochastic sampling.
% \cite{Urban2018} introduce a formal qualitative property, called \textit{Input Data Usage}, to determine whether an input is used by a program or not.
% Their work establishes the theoretical foundations for understanding the usage of input variable.
% Building upon these foundations, subsequent studies~\cite{Urban2020,Mazzucato2021} extend the qualitative input data usage property through static analysis.
% These studies focused on computing the qualitative property as an indicator of bias in neural network models.
% Detecting bias is essential for assessing fairness and discriminatory behavior within these models.
% %
% In our work, we further expand on this framework by considering quantitative properties of input data usage.
% While the qualitative property provides insights into the presence or absence of bias in neural networks, our work offers more flexibility.
% This extension, in the context of fairness verification, enables the identification of input variables more prone to bias.
% This knowledge facilitates a more efficient and targeted analyses, allowing researchers to leverage fairness techniques directly on the variables at a higher risk.
% Without our contribution, identifying biased variables would require running the qualitative analysis separately on each variable, which is computationally intensive and time-consuming.
% %
% % Accordingly, whenever our quantitative prototype yields an impact value of 0, it holds that the input variable under consideration is definitely fair.
% However, it is important to note that input data usage, as developed by~\cite{Urban2018} for programs beyond neural networks, is robust enough to account for the effect of non-determinism. In our prototype, specifically designed for neural network verification, we do not consider non-determinism.
% Therefore, if our quantitative analysis returns an impact value of 0, it holds that the input variable under consideration is definitely not used, but the other implication may not hold.
% It would be interesting to explore the development of a new impact definition capable of resolving non-determinism within our generic framework.
% %
% Furthermore, recent developments~\cite{Ranzato2020,Ranzato2021,Pal2022} are currently targeting other machine learning models, such as decision trees and support vector machines, developing formal methods to discover the usage of input variables.
% Quantitative analyses have the potential to enhance these techniques by leveraging a broader spectrum of possible results.

% \subsection*{Quantitative Information Flow}

% \begin{description}
%   \item[\cite{Gray1991,Denning1982}] first introduced, in a pioneering work, quantitative information flow. Its goal is to measure the amount of information leaked about a secret by observing program computations.
%   \item[\cite{Clark2007}] proposed a first static analysis to quantify interference in a simple imperative language. They quantify leakage in terms of Shannon's information theory.
%   \item[\cite{Malacaria2007}] improved precision of information-theoretic models of programs by focusing on a semantics for looping constructor. Previous series of papers~\cite{Clark2002,Clark2005a,Clark2005b,Clark2007} were overly conservative treatment of looping constructor.
%   \item[\cite{Smith2009}] claimed that Shannon entropy is the wrong measure, because it fails to accurately capture the probability of the attacker first guess at the high data is correct. Another measure is proposed, min-entropy.
%   \item[\cite{Clarkson2009}] developed a new model that that describes how attacker beliefs change due to the attacker's observation of the execution of a non-deterministic program. Another measure is proposed since Shannon mutual information does not appear to result in a theory that allows compositional reasoning.
%   \item[\cite{Phan2012}] proposed a static analysis based on symbolic execution to quantify information leaks.
%   \item[\cite{Alvim2014}] began to address the problem that different pieces of high-security data might be worth more than others, not all leaked bits are equal.
%   \item[\cite{Yasuoka2014}] unified and extended the previous results on the hardness of checking and inferring quantitative information flow
%   \item[\cite{Assaf2016}] improved the traditional techniques of approximating and counting the reachable states in presence of low inputs with the cardinal abstraction.
%   \item[\cite{Assaf2017}] static analysis in the abstract interpretation framework to quantify information leakage employing a fixpoint characterization of hypercollecting semantics.
%   \item[\cite{Cousot2019}] summarized various formal verification techniques for dependencies among input variables of programs into a value dependency static analysis which can be further abstracted to provide insights about information flow, non-interference, and taint analyses to name a few.
%   \item[\cite{Zhang2022}] developed a \textit{strongest-postcondition-style} calculus to enable reasoning about the flow of quantitative information.
%   \item[\cite{Henzinger2023}]generalized the hierarchy of safety and liveness properties to quantitative safety and liveness studying relations among these new classes of properties.
% \end{description}

% Our framework draws inspiration by the works of \sidetextcite{Urban2018}, who introduces a backward analysis for  the qualitative property of ``input data usage,'' and of \sidetextcite{Barowy2014}, which explores the data usage with a stochastic approach for debugging spreadsheet applications.
% Here, we further advance by considering quantitative properties of input data usage.
% While the qualitative property provides insights into the usage or not of program inputs, our work offers more flexibility providing a quantification of such a usage.
% Conversely, our quantitative framework is derived from the qualitative input data usage property, which bears similarities with information flow properties, we drew inspiration from this field to construct our framework.

In quantitative information flow, other works include \sidetextcite{Chothia2014}, which proposed a statistical approach to quantify information leakage for Java programs. \sidetextcite{Phan2012,Phan2014,Saha2023} employed symbolic execution techniques and model counting to obtain sound bounds on the entropy of programs.
Other static analyses, \eg, \sidetextcite{Assaf2017} and \sidetextcite{Clark2007}, are based on abstract interpretation.
\sidetextcite{Girol2024} put forward a quantitative version of non-explotability, called robust reachability, to measure the difficulty of triggering bugs via symbolic execution and a variant of model counting.
%
% A series of papers~\cite{Clark2005a,Clark2005b,Clark2007,Malacaria2007} proposed static analyses to verify quantitative interference using Shannon's information theory and semantics of looping constructors, refining information-theoretic models.
% However,~\cite{Smith2009} argued against the extent of the Shannon entropy measure, proposing min-entropy as more accurate to measure the probability that an attacker guesses private data correctly in the first attempt.
% \cite{Clarkson2009} focused on non-deterministic program and proposed and variation of the Shannon entropy measure.
% Various static analysis techniques have been developed mainly based on abstract interpretation and symbolic execution~\cite{Phan2012,Assaf2017}.
% As a result, there is a need of a generic framework parametric in the definition of the quantitative measure.
% An unification proposed by~\cite{Yasuoka2014} extended previous results on inferring quantitative information flow.
% More recently,~\cite{Zhang2022} developed a calculus based on strongest-postcondition-style allowing quantitative reasoning of information flow, and~\cite{Henzinger2023} generalized the hierarchy of safety and liveness properties to quantitative safety and liveness, exploring relationships among these new property classes.
% %
%
...
For example, \sidetextcite{Assaf2017} counts how many bits of information of input variables are used to compute the result; \sidetextcite{Smith2009} computes the probability of guessing the value of a private variable from the value of input variables; and \sidetextcite{McCamant2008} retrieves the channel capability, which is the worst-case possible for information leakages. Most of these approaches are based on the concept of entropy measures, which are orthogonal to our approach.
Recent developments, \sidetextcite{Zhang2022} developed a calculus based on strongest-postcondition-style allowing quantitative reasoning of information flow, and \sidetextcite{Henzinger2023} generalized the hierarchy of safety and liveness properties to quantitative safety and liveness.
Instead, we present a more fine-grained analysis, which discovers the impact of each input variable separately.
Recent developments \sidetextcite{Zhang2022} developed a calculus based on strongest-postcondition-style allowing quantitative reasoning of information flow, and \sidetextcite{Henzinger2023} generalized the hierarchy of safety and liveness properties to quantitative safety and liveness. In contrast, our quantitative framework, inspired by the qualitative input data usage property, diverges in the information measured. Our analysis focuses on numerical aspects, assessing the impact of input variables on program outcomes, distinguishing it from traditional information flow approaches that count bits or probabilities.

Qualitative input data usage and non-interference relates also with \textit{Data Leakage}~\sidecite[][Chapter~4]{Brownlee2020}.
...
The series of papers developed by \sidetextcite{Drobnjakovic2024,Subotic2022a,Subotic2022b} focuses on verifying data leakage within code notebooks and external libraries such as NumPy or pandas, bringing new challenges in the topic of formal verification.
...
\sidetextcite{Clarkson2010} defined properties of (the semantics of) programs as ``hyperproperties.''
...
In \textcite{Clarkson2010}, there is a categorization of hyperproperties into safety and liveness, respectively hypersafety and hyperliveness. The $k$-hypersafety properties are those that need sets of $k$ traces to be disproved, this category can be verified with traditional techniques for safety properties by $k$-times self-composition \sidecite{Barthe2011,Terauchi2005}.
Note that, direct approaches through self-composition can be computationally expensive \sidecite{Antonopoulos2017}.
In our work, we avoid categorizing the properties of programs as hyperproperties as we do not a different abstract interpretation theory to verify them \sidecite{Cousot2019}. The classical approach \sidecite{Cousot1977} is sufficient. However, observe that the $\defbound$-bounded impact property can be seen as a hyperproperty. Specifically, when applied to the \outcomesname{} quantifier, $\boundedoutcomes$ is a $(k+1)$-hypersafety properties, as they require $k+1$ traces to be disproved, \ie, showing that an impact is greater than $\defbound$. While, when applied to the \rangename{} quantifier, $\boundedrange$ is a 2-hypersafety property, as it requires 2 traces to be disproved, \ie, only two traces holding extreme values for the output variable are sufficient to disprove the property. Finally, the $\boundedqused$ property is a hyperliveness property, thus generally harder to verify than hypersafety properties.
...
Probabilistic programs \sidecite{Gordon2014, Meent2018} are in the form of usual imperative (or functional) programs with additional constructs to model random variables and probabilistic choices.
...
% For instance, Gorinova's work \sidecite{Gorinova2019,Gorinova2022} focuses on static analysis for probabilistic programs, in such setting, analyzing the dependencies between variables in a probabilistic program improving the robustness and reliability of the inference algorithm.
% Recent work \sidecite{Stefanakos2021,Stefanakos2023} synthesized a Markov-chain model of a program via probabilistic model checking, computing a quantitative model of the probability of program transitions. Notably, their approach can predict how well the code would react to changes such as different hardware specifications and profile usage.
% Similarly, \sidetextcite{Todoran2022} translates concurrent programs into corresponding probabilistic models that capture the behavior of programs and that can be verified using probabilistic model checking.
% Applied to our work, probabilistic programming could be used to model the uncertainty in the input data and the program's behavior.
% The development of quantifiers based on probabilistic programs could provide statistical guarantees on the impact of input variables on the program's output.
% \denis{Maybe something like this: While this specific model counting problem has been studied in AI research fields such as Bayesian networks, knowledge representation and probabilistic planning, its use within the context of formal verification presents a new set of challenges.}
% \denis{cite Cousot Probabilistic Abstract Interpretation}
%
Recently, the verification of probabilistic programs made significant advancements.
Martingale theory certificates have been instrumental in automating the proof of quantitative termination and safety in probabilistic programs \sidecite{Chatterjee2017,Chatterjee2022,Takisaka2021,Wang2024}.
Probabilistic programs have also been studied with cost analyses \sidecite{Chatterjee2024,Wang2021}.
Other approaches involve the weakest pre-expectation calculus \sidecite{Kaminski2016}, a probabilistic extension of the classical weakest precondition calculus. This method, while highly expressive, requires user-provided invariants.
...
In our settings, we assume non-determinism to be uniformly distributed, which is a simplification from probabilistic programs. However, by applying probabilistic semantics \sidecite{Cousot2012a}, we could extend our quantifiers to probabilistic measures, providing statistical bounds on the impact of input variables on the probabilistic program's output.
