...
\begin{definition}[Maximal Trace Semantics with the Global Loop Counter]\labdef{counter-semantics}
  ...
  \denis{fix, now is reachability, not maximal trace}
\begin{align*}
...
\begin{definition}[Dependency Semantics]\labdef{backwardreachability-semantics}
...
\begin{example}
  We consider the \refprog*{add}[*-6], where states $\state$ are tuples $\langle p, m, n, nit\rangle$, respectively for the variables \texttt{p}, \texttt{m}, \texttt{n}, and the global loop counter $\counter$.
  ...
  \]
  % For brevity, we consider states as $\state = \setdef{\langle p, m, n\rangle}{p, m, n\in[0, u]}$,
  Then, from \refdef*{concretization-of-conjunctions-of-linear-constraints}[*-19], the concretization of the abstract element $\texttt{p} = \counter$ is:
  \begin{align*}
...
These attacks, including those based on power consumption \sidecite{Kocher1999} and speculative executions \sidecite{Kocher2019,Lipp2018}, can leak sensitive information without physical tampering \sidecite{Wong2005}.
...
Recent advancements in termination analysis have a significant impact in the literature, particularly those grounded in transition invariants as introduced in \sidetextcite{Podelski2004}. Notably, the \textsc{Terminator} analyzer \sidecite{Cook2006} constructs iteratively these invariants to prove termination.
