...
We now derive the \emph{backward co-reachability semantics} $\backwardreachabilitysemantics$ as an abstraction of the dependency semantics.
...
\refsec{conjunctions-of-linear-constraints}
...
\denis{Todo.}
  \end{proof}
%
Thus, by \nrefthm*{soundness} it holds that our static analysis is sound when employed to verify the property $\boundedrange$.
...
While quantitative analyses theoretically could infer similar information, their entropy measures are not designed for this purpose (\cf{} \refsec{sec:nfm24-related-work}), and they usually require $k$-times self-composition \sidecite{Barthe2011,Terauchi2005}, which is computationally expensive \sidecite{Antonopoulos2017}.
...
\paragraph{Input Data Usage.}
The syntactic dependency analysis proposed by~\sidetextcite{Urban2018} can be seen as the qualitative counterpart to our quantitative definition.
It holds that whenever the measured impact is zero, the input variable is definitely unused, \wrt\ the number of iterations of the loop.
Indeed, by considering only the syntactic dependencies, we would have obtained an abstraction of the input data usage property.
