...
Similarly to \refdef*{bounded}, we define the $\defbound$-impact property, which quantifies the impact of input variables on the global number of iterations of a program.
...
As a design choice, we choose the \rangename{} notion among the notions defined in \refsec{k-bounded-impact-property}.
...
\end{lemma}
\begin{proof}
  The proof is similar to the proof of \reflemma*{range-validation}.
\end{proof}
...
In principle, our abstract domain could be any convex abstract domain subsumed by the polyhedra domain~\sidecite{Cousot1978}, such as the interval domain, octagon domain~\sidecite{Min_e2006a}, or the polyhedra domain itself.
...
\end{equation}
%
The soundness condition, \cf{} \refeq{sound-abstract-dependency-semantics}, allows any sound global loop bound analysis $\counterbackwardsemanticsnoparam$ to verify the $\defbound$-bounded impact property $\bounded$.
