...
For instance, the \emph{non-exploitability} property \sidecite{Parolini2024} is an instance of abstract non-interference where the output states are abstracted to a flag that indicates whether the program reached an error state or not.
...
%
\begin{definition}[Right-Left Adjoints for the Output Abstraction Semantics]\labdef{right-left-adjoints-for-the-output-abstraction-semantics}\labdef{abstraction-output-semantics}\labdef{concretization-output-semantics}
\begin{align*}
...
In the rest of the thesis, we abuse the notation and refer to $\unusedwrappername$ for its dependency abstraction.
