...
\begin{definition}[Unused \cite{Urban2018}]\labdef{unused-predicate}
...
\marginnote[*-2]{
\begin{definitionshort}[Abstract Non-Interference \cite{Giacobazzi2018}]\labdef{ani-predicate}
...
\begin{definition}[Output Observer \cite{Giacobazzi2018}]\labdef{output-observer}
...
\begin{example}\labexample{ani-non-determinism}
  ...
  \marginnote{
    Note that, the abstract non-interference predicate $\aniwrapper$ instantiated with the input-output abstractions of \refexample{ani-non-determinism} is the standard non-interference predicate without abstractions.
  }
  \end{example}
...
\begin{proposition}[Abstract Non-Interference Equivalence]\labprop{ani-predicate-equivalence}
  ...
  \begin{align*}
    ...
    \end{cases}
  \end{align*}
  then, it holds that, for any output observer $\aniobserver$:
  \begin{align*}
...
\begin{proof}
  ...
  We need to show that there exists a trace $\deftrace'\in\tracesemantics$ such that: $(i)$ $\retrieveinput{\deftrace'}(\definputvariables) = \defvalue$, $(ii)$ $\retrieveinput\deftrace \stateeq{\inputvariableswithoutw} \retrieveinput{\deftrace'}$, and $(iii)$ $\aniobserver(\retrieveoutput\deftrace) = \aniobserver(\retrieveoutput{\deftrace'})$.
  ...
  Hence, for all traces $\deftrace'\in\tracesemantics$ we assume $\retrieveinput{\deftrace'}(\definputvariables) = \defvalue$ and $\retrieveinput\deftrace \stateeq{\inputvariableswithoutw} \retrieveinput{\deftrace'}$, respectively $(i)$ and $(ii)$.
  ...
  For any trace $\deftrace\in\tracesemantics$ and value $\defvalue\in\valuesofvariables$, it holds that whenever $(a)$ $\retrieveinput{\deftrace}(\definputvariables) \neq \defvalue$, then it exists a trace $\deftrace'\in\tracesemantics$ such that $(b)$ $\retrieveinput{\deftrace'}(\definputvariables) = \defvalue$, $(c)$ $\retrieveinput\deftrace \stateeq{\inputvariableswithoutw} \retrieveinput{\deftrace'}$, and $(d)$ $\aniobserver(\retrieveoutput\deftrace) = \aniobserver(\retrieveoutput{\deftrace'})$ hold.
  To prove $\aniwrapper$, we assume that for any two traces $\deftrace, \deftrace'\in\tracesemantics$ it holds that $\retrieveinput{\deftrace} \stateeq{\inputvariableswithoutw} \retrieveinput{\deftrace'}$, otherwise $\aniwrapper$ vacuously holds. We need to show that the output abstraction of the two traces match, \ie, $\aniobserver(\retrieveoutput\deftrace) = \aniobserver(\retrieveoutput{\deftrace'})$.
...
\end{marginfigure}
  ...
  The two would also be equivalent by considering a concrete semantics where for each input a set of states is returned.
  In this case, the abstract non-interference predicate would consider two identical outputs when their set of states is the same.
\end{example}
...
\marginnote[*-2]{
\begin{definitionshort}[Non-Exploitability \cite{Parolini2024}]\labdef{ne-predicate}
