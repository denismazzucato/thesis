...
\end{tikzpicture}
\caption{Graphical representation of the trace semantics of the \refprog{school-year} considering only the variables \texttt{science} and \texttt{passing}.}
...
\end{example}
  ...
  \begin{marginfigure}[*2]
    \begin{tikzpicture}[scale=0.8]
      ...
      % \draw[decorate,decoration={brace,amplitude=8pt,mirror,raise=4pt},yshift=0pt,xshift=-0.1pt] (-1.1,2.2) -- (-1.1,-0.2) node [black,midway,xshift=-0.7cm,rotate=90] {$\tracesemanticsnoparam\semanticsof{\texttt{is\_passed}_\texttt{science}}$};
  \end{tikzpicture}
  \caption{Trace semantics of \refprog{parity} without parity abstraction applied to the output states.}
  \labfig{parity}
  \end{marginfigure}
...
\begin{example}
  Let us consider the simplified trace semantics of the \refprog{school-year}, considering only the variables \texttt{science} and \texttt{passing}.
...
\begin{marginfigure}
  ...
  \begin{tikzpicture}
    ...
    \node (unused) at (1,-1) {$\unusedtag$};
    \node (overwritten) at (0,-2) {$\overwrittentag$};
...
\begin{example}
  Consider again \refprog*{school-year}[*-2]. The syntactic dependency analysis $\synsemanticsnoparam\semanticsof{\texttt{school-year}}$ starts from the stack $\langle \usagemap_0 \rangle$ where $\usagemap_0$ maps the output variable $\texttt{passing}$ to $\usedtag$ and the rest to $\unusedtag$.
