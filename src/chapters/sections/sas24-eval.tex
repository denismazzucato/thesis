...
\begin{table}[p]
  ...
  \centering
  \caption{Input composition of the \bignum{} library. The variables \textsc{Safe} $\nominalvariables$ are \seabornGreen{highlighted in green}, while the variables \textsc{Numerical} $\numericalvariables$ \seabornRed{in red}. No numerical variable should be \textsc{Maybe Dangerous}.}
...
%
Column \textsc{Safe} $\nominalvariables$ reports the nominal size variables (called \seabornGreen{$s_i$}), column \textsc{Numerical} $\numericalvariables$ reports the numerical variables (called \seabornRed{$n_i$}, where $i$ is the index of the variable as they appear in the function signature).
