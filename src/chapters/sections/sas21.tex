...
For the rest of this chapter, the letter $\defmodel$ denotes a neural network model. The trace semantics of a neural network model, denoted by $\tracesemanticsnoparam\semanticsof{\defmodel}\in\tracetype$, is obtained by propagating a given input vector, step by step, through the list of nodes until the output layer is reached.
...
\index{Forward Semantics!Neural Networks}
...
\index{Backward Semantics!Neural Networks}
...
\index{Abstract Domain!Neural Networks}
\index{Neural Networks!Verification}
...
\index{Impact Quantifier!$ \changes $}
...
\index{Property Validation!$ \changes $}
...
\index{Impact Quantifier!$ \qlibra $}
...
\siderefbox{def}{output-abstraction-semantics}
\index{Property Validation!$ \qlibra $}
...
\index{Impact Quantifier!Abstract Implementation!$ \abstractchanges $}
...
\index{Sound Implementation!\abstractchanges}
...
\index{Soundness!\abstractchanges}
...
\index{Impact Quantifier!Abstract Implementation!$ \abstractqlibra $}
...
\index{Sound Implementation!\abstractqlibra}
\begin{lemma}[$\abstractqlibra$ is a Sound Implementation of $\qlibra$]\lablemma{abstractqlibra-is-sound}
  Let $\definputvariables\in\setof\inputvariables$ be the set of input variables of interest, $\abstractdomain$ the abstract domain, $\backwardsemanticsnoparam$ the family of semantics, and $\buckets\in\vectorbuckets$ the starting output buckets.
  Whenever the following conditions hold:
  \begin{enumerate}[label=(\roman*)]
...
\index{Soundness!\abstractqlibra}
...
\begin{example}
  \reffig{fair-partitions} show a potential partition of the input space into $P_1$ and $P_2$ and $P_3$, fair for the input variables $\definputvariables=\{y\}$.
  Note that, the outcomes of the neural network (represented by colors in \reffig{fair-partitions}) do not play a role in the definition of fair partitions.
  Indeed, an input partition $P$ is fair with respect to the input variable $y$ if all possible permutations of the $y$-axis of any input value $(x_0, y_0) \in P$ result in a point $(x_0, y')$ that still belongs to $P$.
\end{example}
...
\index{Soundness!\parallelqlibra}
\begin{theorem}
...
The most common investigated problem in this domain is to verify the robustness of neural networks against minimal input perturbations \sidecite{Goodfellow2016}.
This research area is highly diverse and work in the field usually employ a variety of techniques, including SMT-based methods \sidecite{Katz2017,Katz2019,Pulina2011a,Pulina2011b,Pulina2012}, mixed-integer programming \sidecite{Botoeva2020,Lomuscio2017,Tjeng2019}, branch-and-bound algorithms \sidecite{Bunel2018,Palma2021,Ehlers2017}, over-approximation of non-linear activation functions \sidecite{Singh2019,Weng2018,Zhang2018b}, and symbolic propagation \sidecite{Henriksen2020,Wang2018b,Wang2021b}.
...
See \sidetextcite{Chen2024} for a comprehensive survey on fairness testing.
On the other hand, some works \sidecite{Bastani2019,Ghosh2021,Ghosh2022} provide stochastic guarantees on the fairness of machine learning models, while others employ abstract interpretation \sidecite{Urban2020,Mazzucato2021} and SMT based techniques \sidecite{Biswas2023,George2020} to verify fairness properties in neural networks, offering formal guarantees.
Additionally, \sidetextcite{Albarghouthi2017b} encodes the fairness problem as a probabilistic program property, verified using an SMT solver, and \sidetextcite{Sun2021} similarly employ probabilistic verification by learning Markov Chains.
Furthermore, \sidetextcite{Albarghouthi2019} proposes a technique to repair biases in neural networks.
Finally, the most similar work to our quantifier \qlibraname{} is \sidetextcite{Kim2025}, which quantifies the fairness of input variables based on interval symbolic propagation.

Recent formal developments~\sidecite{Ranzato2020,Ranzato2021,Pal2024} are expanding this research to other machine learning models, such as decision trees and support vector machines, by developing formal methods to analyze input variable usage. Quantitative analyses have the potential to enhance these techniques by leveraging a broader spectrum of possible results.
