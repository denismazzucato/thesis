...
It follows that whenever the program is deterministic, the traces concretized from the backward analysis are deterministic as well.
...
\begin{lemma}[$\abstractchanges$ is a Sound Implementation of $\changes$]\lablemma{abstractchanges-is-sound}
  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item \label{nioa3} $\abstractdomain$ is an exact backward semantics, \cf{} \reflemma{exact-backward-analysis},
    \item \label{nioa4} $\abstractdomain$ is equipped with the abstract operator $\abstractdomainproject$ that projects away the input variables $\definputvariables$ from the abstract values,
  \end{enumerate}
  then it holds that:
  \[\abstractchanges(\presfrombuckets, \buckets) \GE \changes(\presfrombuckets, \buckets)\]
\end{lemma}
...
In order to determine whether how fair the input variables $\definputvariables$ of a neural-network model are, we can independently verify, for each of its possible target class, the volume of input space
