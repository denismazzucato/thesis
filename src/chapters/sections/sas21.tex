...
\refalg{backward-neural-networks} depicts the backward analysis of neural network models, where the abstract operator $\abstractdomainbackwardaffine$ computes the result $\defabstractvalue$ of the analysis of the
...
\begin{marginfigure}
  ...
  \begin{tikzpicture}[scale=0.9]
    ...
    \fill[color=seabornGreen, opacity=0.5] (0.5,0) -- (4.5,0) -- (4.5,1.5) -- (0.5,1.5) -- cycle;
    \draw[color=seabornGreen, ultra thick] (0.5,0) -- (4.5,0) -- (4.5,1.5) -- (0.5,1.5) -- cycle;
...
\begin{marginfigure}
  ...
  \begin{tikzpicture}[scale=0.9]
    ...
    % box around the function, fill with opaque \green and the contour in \green
    \fill[color=seabornGreen, opacity=0.5] (0.5,0) -- (4.5,0) -- (4.5,1.5) -- cycle;
    \draw[color=seabornGreen, ultra thick] (0.5,0) -- (4.5,0) -- (4.5,1.5) -- cycle;
  ...
  \begin{tikzpicture}[scale=0.9]
    ...
    % box around the function, fill with opaque \green and the contour in \green
    \fill[color=seabornGreen, opacity=0.5] (0.5,0) -- (0.5,-2.5) -- (4.5,1.5) -- cycle;
    \draw[color=seabornGreen, ultra thick] (0.5,0) -- (0.5,-2.5) -- (4.5,1.5) -- cycle;
...
\begin{marginfigure}
  ...
  \begin{tikzpicture}[scale=0.9]
    ...
    % box around the function, fill with opaque \green and the contour in \green
    \fill[color=seabornGreen, opacity=0.5] (0.5,0) -- (0.5,-0.95) -- (4.5,0.55) -- (4.5,1.5) -- cycle;
    \draw[color=seabornGreen, ultra thick] (0.5,0) -- (0.5,-0.95) -- (4.5,0.55) -- (4.5,1.5) -- cycle;
...
\reflemma{changes-monotonic} and \reflemma{changes-validation} show that the $\changesname$ impact notion can be used to certify that a program has impact of \emph{at most} $\defbound$, meaning that the program output changes at most $\defbound$ times when the input variables vary.
% In case of non-deterministic set of traces, the $\changesname$ impact notion would not be meaningful, as the number of stable regions leading to a different output value would be always infinite.


\subsection{The \qlibraname{} Impact Notion}[\qlibraname]
\labsec{qlibra-impact-notion}

The second notion introduced in this section is the \qlibraname{} impact notion, which stands for \underline{q}uantitative \underline{a}bstract \underline{u}nused.
...
\reflemma{qlibra-monotonic} and \reflemma{qlibra-validation} show that the $\qlibraname$ impact notion can be used to certify that a program has impact of \emph{at least} $\defbound$, meaning that the input variables $\definputvariables$ of a network are fair for at least $\defbound$ fraction of the input space.
...
The core idea of the $\changesname$ impact notion is to group together abstract elements representing distinct stable regions. Convex abstract domains, such as polyhedra, octagons, or intervals (introduced in \refsec{abstract-domains}), are inadequate for representing multiple distinct stable regions because they also include the intermediate points. Therefore, the abstract domain $\abstractdomain$ used in the backward analysis $\backwardsemanticsnoparam$ needs to employ disjunctive sets, which allow for the representation of distinct stable regions.
%
...
From the polyhedra domain $\polyabstractdomain$, the function $\abstractdomainproject$ computes the Fourier-Motzkin elimination algorithm~\sidecite{Dantzig1973} to project away the input variables $\definputvariables$.
...
\end{lemma}
\begin{proof}
  Note that, by \refdef{qlibra}, $\qlibra(\defsetoftraces)$ computes the volume of input space that does not use the input variables $\definputvariables$. It does so by partitioning the set of given traces that are perturbations of the input variables $\definputvariables$, \cf{} the set $\defsetoftraces_{\retrieveinput{\deftrace}}$ on the side. By definition, these partitions are fair partitions by \refdef{fair-input-partition}.
  ...
  As a consequence, the $\qlibra$ impact notion could be written as the sum of the impacts of the minimal fair partitions, \ie, $\qlibra(\tracesemantics) = \sum_{\deftrace\in\tracesemantics} \qlibra(\setdef{\deftrace'\in\tracesemantics}{\retrieveinput{\deftrace'} \stateeq{\inputvariableswithoutw} \retrieveinput{\deftrace}})$.
...
\begin{definition}[Parallel Abstraction Semantics]\labdef{parallel-abstraction-semantics}
  ...
  \marginnote{\begin{math}
    \partition\tracesemanticsnoparam\definputpartition \DefeQ \setdef{\defstate}{\inputoutputtuple\defstate\in\tracesemanticsnoparam \land \retrieveinput{\defstate}\in\definputpartition}
  \end{math}}
  \begin{align*}
...
QAUnused%${}^\parallel_{\defmodel, \abstractdomain}$%(%$\definputvariables$%, %$\analysisinputspace$%, %$\tune$%):
  %$\upperbound$%, %$\lowerbound$%, %$\maxupperbound$%, %$\minlowerbound$%, %$\heuristic$% = %$\tune$%
  %\F%, %\E%, %\C% = %$\emptyset$%, %$\emptyset$%, %$\emptyset$%
  %$\definputpartitions$% = %$\{\analysisinputspace\}$%
  do
    while %$\definputpartitions \neq \emptyset$% do
      %\I% = %$\definputpartitions$%.pop()
      %$\a$%, %$\p$% = Forward%${}_{\defmodel, \abstractdomain}$%(%\I%)
      if %$\a$% is unique then
        %\C% = %$\C \cup \{\p \mapsto \I\}$%
      else if %$\cardinalitynospaces\defmodel - \cardinalitynospaces\p \le \upperbound$% then
        %\F% = %$\F \cup \{\I\}$%
      else if %$\cardinalitynospaces\I \le \lowerbound$% then
        %\E% = %$\E \cup \{\I\}$%
      else
        %$\definputpartitions$% = %$\definputpartitions$% %$\cup$% Partition%${}_{\inputvariableswithoutw}$%(%\I%)
    %$\definputpartitions$% = %\E%
    %$\upperbound$%, %$\lowerbound$% = %$\heuristic$%(%$\upperbound$%, %$\lowerbound$%)
  while %$\upperbound \neq \maxupperbound$% and %$\lowerbound \neq \minlowerbound$%
  %\E% = %$\definputpartitions$%
  %\B% = %$\emptyset$%
  for %$\p\mapsto \definputpartitions$% in %\F% do
    %\O% = %$\emptyset$%
...
The next chapter will present the experimental results of the \changesname{} and \qlibraname{} impact notions applied to neural networks.
