...
\emph{Dans ce chapitre, nous introduisons le contexte de l'analyse formelle des réseaux de neurones et nous définissons formellement le concept des réseaux de neurones profonds feed-forward à des fins de classification.
Nous introduisons deux notions d'impact quantitatif : la notion d'impact \changesname{}, qui cible les répétitions de changements dans le résultat, et la notion d'impact \qlibraname{}, qui mesure le degré de biais d'un réseau de neurones. Nous présentons ensuite l'implémentation abstraite des deux notions d'impact, respectivement appelées $\abstractchangesname$ et $\abstractqlibraname$, et nous montrons comment valider la propriété d'impact bornée par $\defbound$ pour les deux notions. Enfin, nous présentons l'implémentation parallèle $\parallelqlibraname$ pour une vérification efficace des réseaux de neurones selon la notion d'impact \qlibraname{}.
Ce chapitre est basé sur le travail présenté au 28ème Symposium sur l'Analyse Statique (SAS 2021)~\sidecite{Mazzucato2021}.}
...
For the rest of the chapter, the letter $\defmodel$ denotes a neural network model (similarly to the letter $\defprogram$ for generic programs).
The trace semantics of a neural network model is then $\tracesemanticsnoparam\semanticsof{\defmodel}$.
...
The abstract interpretation of a neural-network model is the propagation of an abstract element through each neuron of the network, over-approximating the possible states the model may reach.
The analysis either goes forward, from the input layer to the output layer, or backward, from the output layer to the input layer.
The forward analysis receives an abstract element representing the input values and returns an abstract element representing an over-approximation of the reachable target classes.
On the other hand, the backward analysis receives an abstract element representing the target classes and returns an abstract element representing an over-approximation of the input values that may lead to one of the given target classes.

When a neural network is given an input vector, the network computes the value of each node in the network via an affine transformation and the application of an activation function, in our case a \relu{} function.
The \relu{} flattens the negative values to zero, and keeps the positive values unchanged.
We say that the \relu{} node (or better, the node after the application of the \relu{} function) is \emph{active} if the value is positive, and \emph{inactive} otherwise.
The active or inactive flag of a node is called its \emph{activation status}.
The set of activation statuses of all the nodes in the network is called the \emph{activation pattern} of the network, inducing a \emph{path} in the neural network from input space regions where the network behaves similarly.

An \emph{abstract activation pattern} gives an activation pattern only to a subset of \relu{} nodes, hence representing a set of activation patterns.
\relu{} nodes whose corresponding flag does not have a fixed value are considered to have an unknown activation status.
The abstract activation pattern is very useful for the abstract interpretation of neural networks, as the knowledge that a node has an inactive status can be used to prune the search space of the analysis, and thus reduce the computational cost.
Specifically, the \relu{} activation function is much simpler to handle when the activation status is known, as the \relu{} function is the identity function for active, and the constant zero for inactive nodes.
The abstract activation function can easily be collected from a forward analysis of the network, whenever the over-approximation of input values of a node is greater than zero, the node is considered active, whenever it is less than zero, the node is considered inactive, otherwise unknown.

Hence, given an abstract domain $\abstractdomain$ with sound forward and backward operators for the affine and \relu{} operations, the forward analysis can be defined as the network computation where we replace concrete operators with their abstract sound counterparts.
\reffig{forward-neural-networks} presents a forward abstract analysis of neural-network models, parametric on the abstract domain $\abstractdomain$ and an abstract element $x^\natural$ representing the input values.
Specifically, the abstract operator $\abstractdomainforwardaffine$ computes the affine transformation of the given node, and the abstract operator $\abstractdomainforwardrelu$ takes care of the \relu{} activation function and the computation of the activation status of the node.

\marginnote[*-15]{\denis{fix the name for programs and algorithms}}
\begin{marginlisting}
...
forward(%$\abstractdomain$%, %$x^\natural$%):
  p = %$\emptyset$%
  for %$i$% = 1 up to %$\lastlayerindex$%:
    for %$j$% = 1 up to %$\cardinalitynospaces{\hiddenlayer{i}}$%:
      %$x^\natural$% = %$\abstractdomainforwardaffine\semanticsof{\node}x^\natural$%
      %$x^\natural, p$% = %$\abstractdomainforwardrelu\semanticsof{\node}x^\natural$%
  return %$x^\natural, p$%
\end{lstlisting}
\end{marginlisting}

Conversely, the backward analysis of neural networks is defined as the network computation where we replace concrete operators with their abstract sound \emph{backward} counterparts.
The backward analysis is parametric on the abstract domain $\abstractdomain$ and an abstract element $x^\natural$ representing the target classes.

Both analyses are sound in the sense that they over-approximate the possible states the network may reach.
Indeed, the trace semantics of the network computation is soundly abstracted by the abstract semantics of the forward and backward analyses.
...
Other than the backward semantics $\backwardsemanticsnoparam$, we assume a forward
...
The abstract implementation $\abstractqlibra$ is defined as the sum of the volumes of the fair partitions.
...
Note that, the backward analysis is exact for each partition, thus very expensive.

More specifically, the forward analysis bounds the number of paths that the backward analysis has to explore.
We represent each path by an \emph{activation pattern}, which determines the activation status of every \relu{} operation in the network model.
An activation pattern is a binary vector of length equal to the number of \relu{} operations in the network model.
Each flag in the vector could be either active or inactive depending on the status of the corresponding \relu{} operation.

On the other hand, an \emph{abstract activation pattern} gives an activation pattern only to a subset of \relu{} nodes, hence representing a set of activation patterns.
\relu{} nodes whose corresponding flag does not have a fixed value are considered to have an unknown activation status.

We set up an analysis \emph{budget} by providing an upper bound $\upperbound$ on the number of tolerated \relu{} nodes with an unknown activation status for each element $\definputpartition\in\definputpartitions$, \ie{} the number of paths that are to be explored by the backward analysis in each partition $\definputpartition$.
The forward analysis starts with the trivial partition $\definputpartitions = \{\reducedstate\}$.
...
Finally, the abstract implementation $\abstractqlibra$ computes the sum of the volumes of the fair portion of each partition together.
The next result shows that the abstract implementation $\abstractqlibra$ is a sound over-approximation of the concrete implementation $\qlibra$.
Therefore, it can be used to validate the $\boundedqlibra$ property.

\begin{theorem}
  Given the program $\defprogram$ and the input variable $\definputvariables\in\setof\inputvariables$, an abstract domain $\abstractdomain$, and the budget $\lowerbound, \upperbound$, the abstract implementation $\abstractqlibra$ is a sound over-approximation of the concrete implementation $\qlibra$:
  \begin{align*}
    \abstractqlibra(\multisemantics\buckets, \buckets) = \defbound' \land \defbound' \comparison \defbound \ImplieS \defprogram \satisfies \GENBOUNDED
  \end{align*}
\end{theorem}
