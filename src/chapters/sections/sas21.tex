...
For the rest of this chapter, the letter $\defmodel$ denotes a neural network model (analogous to the letter $\defprogram$ for generic programs). The trace semantics of a neural network model is denoted by $\tracesemanticsnoparam\semanticsof{\defmodel}\in\tracetype$.
Note that, under the models we consider in this thesis, we have that the output of the network is deterministic for each input configuration and that all the traces terminate after a specific number of steps, which is the number of nodes of the network.
...
\begin{example}
Consider a simple neural network with two input and two output variables. The weights and biases connecting the input to the output layer are irrelevant for this example. Therefore, the network states are defined as $\defstate = \setdef{\langle x_{0, 0}, x_{0, 1}, x_{1, 0}, x_{1, 1} \rangle}{x_{0, 0}, x_{0, 1}, x_{1, 0}, x_{1, 1} \in \R}$. For simplicity, we assume all the variables to be real\sidenote{often neural networks normalize inputs to the interval $[0,1]$}.

Given, for instance, the network state $\langle 0, 1, 0, 1 \rangle$, the output observer $\outputobs$ returns the state $\outputobs(\defstate) = \langle 0, 0, 0, 1 \rangle$ to indicate that the target class is the second one, corresponding to $x_{1, 1}$. Given the network state $\langle 0.5, 1, 0.7, 0.2 \rangle$, the output observer returns the state $\outputobs(\defstate) = \langle 0, 0, 1, 0 \rangle$ to indicate that the target class is the first one (the first two variables are the input variables), and so on.
\end{example}
...
Given an abstract domain $\abstractdomain$ with sound forward and backward operators for affine and \relu{} operations, the forward analysis can be defined as network computation using abstract sound counterparts of the concrete operators. \reffig{forward-neural-networks} illustrates a forward abstract analysis of neural network models, parameterized by the abstract domain $\abstractdomain$ and an abstract element $x^\natural$ representing the input values. The abstract operator $\abstractdomainforwardaffine$ computes the affine transformation of a given node, while $\abstractdomainforwardrelu$ handles the \relu{} activation function and determines the node's activation status.

Conversely, backward analysis of neural networks replaces concrete operators with their abstract sound \emph{backward} counterparts. This analysis is also parameterized by the abstract domain $\abstractdomain$ and an abstract element $x^\natural$ representing the target classes.
...
\[
  \forwardconcretization(\forwardsemanticsnoparam)x^\natural \DefeQ
  \nonemptysetof{\setdef{
    \inputoutputtuple{\defstate}
  }{
    \retrieveinput{\defstate}\in\abstractdomainconcretization(\forwardsemanticsnoparam(x^\natural))\land\retrieveoutput{\defstate}\in\abstractdomainconcretization(x^\natural)
    }
  }
  \]
...
\begin{lemma}[Soundness of Forward Neural Network Analysis]\lablemma{soundness-forward-network}
  ...
  \[
    \reduce[\outputsemanticsnoparam\semanticsof{\defmodel}]{\abstractdomainconcretization(x^\natural)} \SubseteQ \forwardconcretization(\abstractdomainforward(\defmodel))x^\natural
  \]
\end{lemma}
\begin{proof}
...
\begin{lemma}[Soundness of Backward Neural Network Analysis]\lablemma{soundness-backward-network}
  ...
  \[
    \reduce[\outputsemanticsnoparam\semanticsof{\defmodel}]{\abstractdomainconcretization(x^\natural)} \SubseteQ \backwardconcretization(\abstractdomainbackward(\defmodel))x^\natural
  \]
\end{lemma}
\begin{proof}
...
In case of non-deterministic set of traces, the $\changesname$ impact notion would not be meaningful, as the number of continuous regions leading to a different output value would be infinite.
...
Importantly, this domain is exact for the property of interest, as it allows us to represent multiple distinct continuous regions, and for feed-forward \relu-activated neural networks.
