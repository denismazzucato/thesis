...
Based on this, formal methods are classified into three categories \sidecite{Cousot2010} (\cf{} \reffig{formal-methods-trade-offs}): \emph{theorem provering}, \emph{model checking}, and \emph{static analysis}.
...
In data science applications, where software involves long pipelines that filter, merge, and manipulate data, programming errors causing input variables to have more or less influence than expected are even more likely to occur.
...
\marginnote[*-10]{\refch{quantitative-input-data-usage} and \refch{showcase} in \arefpart{extensional} are based on the work published at NASA Formal Methods
Symposium (NFM) 2024 \cite{Mazzucato2024b}.}
\marginnote[*-6.7]{\formatmargincitation{Mazzucato2024b}}
...
In this thesis, we focus on quantifying input variables' impact on loop iterations as an indicator of runtime behavior.
